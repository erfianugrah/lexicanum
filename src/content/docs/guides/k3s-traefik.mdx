...
| :---- | :--- | :---------- | :-------- |
| Grafana | `grafana-k3s.erfi.io` | ~~rl-grafana~~, inflight-req, retry | monitoring |
| Prometheus | `prom-k3s.erfi.io` | ~~rl-prometheus~~, inflight-req, retry | monitoring |
| Alertmanager | `alertmanager-k3s.erfi.io` | ~~rl-alertmanager~~, inflight-req, retry | monitoring |
| Jaeger | `jaeger-k3s.erfi.io` | ~~rl-jaeger~~, inflight-req, authentik-forward-auth, retry | monitoring |
| Logpush | `logpush-k3s.erfi.io` | ~~rl-logpush~~, inflight-req, decompress, retry | monitoring |
| Traefik Dashboard | `traefik-dashboard.erfi.io` | ~~rl-traefik-dashboard~~, inflight-req, retry | traefik |
| Traefik Prometheus | `traefik-prometheus.erfi.io` | ~~rl-traefik-prometheus~~, inflight-req, retry | traefik |
| Authentik | `authentik.erfi.io` | ~~rl-authentik~~, inflight-req, authentik-csp, retry | authentik |
| Revista | `erfianugrah.com` | rl-revista, inflight-req, retry | revista |
| ArgoCD (HTTP) | `argocd.erfi.io` | rl-argocd, inflight-req, retry | argocd |
| ArgoCD (gRPC) | `argocd.erfi.io` + gRPC header | rl-argocd, inflight-req, retry | argocd |
| Dendrite | `dendrite.erfi.io` | rl-dendrite, inflight-req, retry | dendrite |
| httpbun | `httpbun-k3s.erfi.io` | rl-httpbun, inflight-req, retry | httpbun |
| Jitsi (from Element) | `jitsi.erfi.io` + Referer match | rl-jitsi, inflight-req, retry | jitsi |
| Jitsi (direct) | `jitsi.erfi.io` | rl-jitsi, inflight-req, retry | jitsi |
| LiveKit JWT | `matrix-rtc.erfi.io/livekit/jwt` | rl-livekit, inflight-req, strip-livekit-jwt, retry | livekit |
| LiveKit SFU | `matrix-rtc.erfi.io/livekit/sfu` | rl-livekit, inflight-req, strip-livekit-sfu, retry | livekit |
| Element (chat) | `chat.erfi.io` | rl-matrix-element, inflight-req, retry | matrix |
| Synapse Admin | `admin.matrix.erfi.io` | rl-matrix-admin, inflight-req, retry | matrix |
| Synapse | `matrix.erfi.io` | rl-matrix-synapse, inflight-req, retry | matrix |
| Maubot | `maubot.erfi.io` | rl-maubot, inflight-req, retry | maubot |
| Headlamp | `headlamp-k3s.erfi.io` | rl-headlamp, inflight-req, retry | headlamp |
| Longhorn | `longhorn.erfi.io` | rl-longhorn, inflight-req, retry | longhorn-system |
| Portainer | `portainer-k3s.erfi.io` | rl-portainer, inflight-req, retry | portainer |
| Portainer Agent | `port-agent-k3s.erfi.io` | rl-portainer-agent, inflight-req, retry | portainer |
...
The Alloy DaemonSet picks up these logs from the Traefik container's stdout (via `/var/log/pods/`), parses the JSON, and sends them to Loki with structured metadata. See the [monitoring stack guide](/guides/k3s-monitoring-stack/#traefik-access-log-enrichment) for the full Alloy config and the label/metadata split.
...
curl -s -o /dev/null -w "%{http_code}" -H "User-Agent: sqlmap/1.0" https://httpbun-k3s.erfi.io/

# Test honeypot path (should return 403)
curl -s -o /dev/null -w "%{http_code}" https://httpbun-k3s.erfi.io/.env
```
