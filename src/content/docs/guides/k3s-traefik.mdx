...
category: kubernetes
featured: true
blurb: "Custom deployment, plugins, middlewares, and Cloudflare Tunnel."
---
...
This guide covers the full setup: disabling the built-in Traefik, deploying a custom one as a raw Deployment manifest, writing and packaging Traefik Go plugins as ConfigMaps, configuring the middleware chain, managing TLS certificates via Cloudflare DNS challenge, routing traffic through Cloudflare Tunnel, autoscaling with Kubernetes Event-driven Autoscaling (KEDA), and piping access logs + traces into the monitoring stack.
...
3. **Custom Resource Definition (CRD) installation**: Traefik CRDs and role-based access control (RBAC) for the KubernetesCRD provider
...
7. **TLS configuration**: TLSOption CRD, Automatic Certificate Management Environment (ACME) DNS challenge via Cloudflare, strong cipher suite
...
11. **Observability**: Access logs (JSON -> Alloy -> Loki with 19 structured metadata fields), tracing (OpenTelemetry Protocol (OTLP) -> Alloy -> Jaeger), metrics (Prometheus with router labels + 7 Alloy-generated counters)
...
digraph {
  ...
  subgraph cluster_traefik {
    label="Traefik v3.6.8\n(traefik ns, 2 replicas)"; style=rounded; fontsize=12; margin=12;
    subgraph cluster_traefik_global {
      label="Global Middlewares"; style=rounded; fontsize=12; margin=12;
      traefik_global_sentinel [label="Sentinel\n(IP + bot + blocklist\n+ rule engine)"];
      traefik_global_headers  [label="Security Headers\n(HSTS, nosniff)"];
    }
    subgraph cluster_traefik_perroute {
      label="Per-route Middlewares"; style=rounded; fontsize=12; margin=12;
      traefik_perroute_rl         [label="rl-*\n(per-service rate limit)"];
      traefik_perroute_inflight   [label="inflight-req\n(100 concurrent/IP)"];
      traefik_perroute_retry      [label="retry\n(3 attempts)"];
      traefik_perroute_decompress [label="decompress\n(gzip body)"];
      traefik_perroute_auth       [label="authentik-forward-auth"];
    }
  }

  subgraph cluster_services {
    label="Backend Services"; style=rounded; fontsize=12; margin=12;
    services_grafana    [label="Grafana\n(monitoring)"];
    services_prometheus [label="Prometheus\n(monitoring)"];
    services_authentik  [label="Authentik\n(authentik)"];
    services_revista    [label="Revista\n(revista)"];
    services_matrix     [label="Matrix stack\n(matrix)"];
    services_jitsi      [label="Jitsi\n(jitsi)"];
    services_others     [label="..."];
  }

  subgraph cluster_monitoring {
    label="Observability"; style=rounded; fontsize=12; margin=12;
    monitoring_alloy  [label="Alloy DaemonSet"];
    monitoring_loki   [label="Loki"];
    monitoring_jaeger [label="Jaeger"];
    monitoring_prom   [label="Prometheus"];
  }

  cf_tunnel -> traefik_global_sentinel [label="HTTPS\n(h2, SNI)", lhead=cluster_traefik];
  traefik_global_sentinel -> traefik_global_headers;
  traefik_perroute_rl -> services_grafana [ltail=cluster_traefik_perroute, lhead=cluster_services];
  traefik_global_sentinel -> monitoring_alloy [label="access logs\n(JSON stdout)", ltail=cluster_traefik];
  traefik_global_sentinel -> monitoring_alloy [label="OTLP traces", ltail=cluster_traefik];
  traefik_global_sentinel -> monitoring_prom  [label="metrics\n(:8082)", ltail=cluster_traefik];
  monitoring_alloy -> monitoring_loki   [label="structured\nmetadata"];
  monitoring_alloy -> monitoring_jaeger [label="traces"];
}
```
...
host eq "logpush-k3s.example.com" and not header["X-Logpush-Secret"] eq "..."
```
...
| :- | :------- | :--------- | :----- |
| r1 | 1 | `ip eq "203.0.113.42"` | `allow` (owner IP bypass) |
| r6 | 2 | `host eq "logpush-k3s.example.com" and not header["X-Logpush-Secret"] eq "..."` | `block` (deny without secret) |
| r2 | 10 | `path contains "/.git" and not ip eq "203.0.113.42"` | `block` |
...
spec:
  plugin:
    sentinel:
      ...
      rules: |
        [
          {"id":"r1","description":"Allow owner IP","expression":"ip.src eq \"203.0.113.42\"","action":"allow","enabled":true,"priority":1},
          ...
        ]
```
...
The format is `<namespace>-<name>@kubernetescrd`. Order matters - sentinel runs first (resolves IP, checks blocklist, scores request, evaluates rules), then security-headers adds HTTP Strict Transport Security (HSTS) and other response headers.
...
`frameDeny`, `browserXssFilter`, and the Content Security Policy (CSP) are intentionally omitted from the global middleware. These are app-specific - Authentik needs its own CSP, Grafana needs iframe support for embedding, etc. Apply those per-route where needed.
...
Dashboards query these structured metadata fields directly instead of using `| json` full-line parsing, which cut response times from 30s+ to 14ms.

Alloy also generates **7 Prometheus counters** via `stage.metrics`, categorized by Sentinel block type:
...
Source: `services/security-dashboard/`. Deploy: `docker build --platform linux/arm64 -t erfianugrah/security-dashboard:latest .` -> `docker push` -> `kubectl rollout restart deployment/security-dashboard -n security-dashboard`. See "Plugin 1: Sentinel" in Part 4 above for the architecture.
