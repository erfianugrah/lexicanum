...
Step 6: Configure k3s control node
...
```mermaid
%%{init: {'theme': 'default', 'themeVariables': { 'primaryColor': '#FFCC00', 'edgeLabelBackground':'#ffffff', 'tertiaryColor': '#FFFFFF', 'lineColor': '#666666', 'arrowheadColor': '#666666', 'fontSize': '14px', 'backgroundColor': 'transparent'}}}%%
graph TB;
    classDef user fill:#FFE4B5,stroke:#333,stroke-width:2px,color:#000;
    classDef kubectl fill:#326CE5,stroke:#333,stroke-width:3px,color:#fff;
    classDef cloudflare fill:#FFA500,stroke:#333,stroke-width:2px,color:#000;
    classDef idp fill:#4285F4,stroke:#333,stroke-width:2px,color:#fff;
    ...
    classDef k3sCluster fill:transparent,stroke:#333,stroke-width:3px,stroke-dasharray: 5 5;
    classDef oidcFlow fill:#E6F3FF,stroke:#0066CC,stroke-width:2px;
    classDef tunnel fill:#FF9999,stroke:#333,stroke-width:2px;

    %% User and kubectl at top
    User((👤 User))
    kubectl[kubectl + kubelogin]
    User --> kubectl
    class User user;
    class kubectl kubectl;

    %% OIDC Authentication Flow - vertical
    kubectl -->|1. OIDC Auth Request| CFAccess[Cloudflare Access<br/>SaaS Application]
    CFAccess -->|2. Redirect to IDP| IDP[Identity Provider<br/>Google/Authentik/etc]
    IDP -->|3. User Authentication| CFAccess
    CFAccess -->|4. JWT Token| kubectl
    class CFAccess cloudflare;
    class IDP idp;

    %% Cloudflare Tunnel - positioned vertically  
    CFTunnel[Cloudflare Tunnel<br/>kubectl.domain.com]
    kubectl -->|5. API Request + JWT| CFTunnel
    CFTunnel -->|6. Policy Check| CFAccess
    CFAccess -->|7. Access Granted| CFTunnel
    class CFTunnel tunnel;

    %% k3s Cluster - reorganized vertically
    subgraph k3s_Cluster["🏢 k3s Cluster"]
        %% Control Plane at top
        subgraph ControlPlane["🎯 Control Plane"]
            Node1("🎯 Node 1<br/>Control Plane<br/>kube-apiserver") 
        end
        class ControlPlane nodes;

        %% Cloudflared deployment
        subgraph cloudflared_deployment["☁️ Cloudflared Deployment"]
            cloudflared("🌐 cloudflared<br/>Tunnel Agent<br/>Metrics: :50000")
        end
        class cloudflared_deployment cloudflared;

        %% Load Balancer layer
        subgraph LB["⚖️ MetalLB Load Balancer"]
            MetalLB_Controller("🎛️ Controller<br/>Node 3")
            MetalLB_Speaker1("📢 Speaker<br/>Node 1")
            MetalLB_Speaker2("📢 Speaker<br/>Node 2") 
            MetalLB_Speaker3("📢 Speaker<br/>Node 3")
            MetalLB_Speaker4("📢 Speaker<br/>Node 4")
        end
        class LB lb;

        %% Ingress layer
        subgraph Traefik_Ingress["🚪 Traefik Ingress Controller"]
            Traefik("🔀 Traefik<br/>IP: 10.68.71.100")
        end
        ...
        subgraph Services["🎯 Services"]
            ...
            Httpbun("🧪 httpbun")
            TraefikDashboard("🎛️ Traefik Dashboard")
        end
        class Services service;

        %% Worker Nodes
        subgraph Workers["⚙️ Worker Nodes"]
            Node2("⚙️ Node 2<br/>Worker")
            Node3("📁 Node 3<br/>Worker + NFS")
            Node4("⚙️ Node 4<br/>Worker")
        end
        class Workers nodes;

        %% Vertical flow within cluster
        cloudflared -->|Tunnel Connection| CFTunnel
        Node1 -->|OIDC Validation| CFAccess
        MetalLB_Controller -->|Assigns VIP| Traefik
        ...
        Traefik -->|Route Traffic| Services
        Node1 -.-> Workers
    end
    class k3s_Cluster k3sCluster;

    %% Main vertical data flow
    CFTunnel -->|8. Forward to k3s API| Node1
    Node1 -->|9. Validate JWT| kubectl
    kubectl -->|10. API Response| User

    %% Configuration annotations
    Node1 -.->|"kube-apiserver-arg:<br/>--oidc-issuer-url<br/>--oidc-client-id<br/>--oidc-username-claim=email<br/>--oidc-groups-claim=groups"| CFAccess

    %% Legend positioned at bottom
    subgraph Legend["📋 Authentication Flow Steps"]
        L1["🔐 Steps 1-4: OIDC Authentication & JWT"]
        L2["🛡️ Steps 5-7: Access Policy Validation"] 
        L3["⚡ Steps 8-10: Secure API Access"]
    end
    class Legend oidcFlow;
```
