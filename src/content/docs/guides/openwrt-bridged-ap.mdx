...
description: Taking a consumer WiFi 6 router (GL.iNet GL-MT6000 class) from vendor firmware to stock mainline OpenWrt as a pure layer-2 access point behind a separate router - the flash and the u-boot recovery that saved it, a deploy script that keeps the box equal to a git repo, the wpad-basic option that silently kills a radio, the dumb-AP settings the OpenWrt docs ask for, and two out-of-band paths back in when the LAN is gone.
author: Erfi Anugrah
category: homelab
---
...
Not knowing either cost forty minutes on the rig, then a reset button pressed on a box that was working. On mainline OpenWrt, holding reset on a running box for more than five seconds is a factory reset and reboot. Do not press it on a box you cannot see.
...
Six UCI files in a repo, one gitignored env file with the WPA keys, and a deploy script, which runs in this order:
...
## Part 5: the dumb-AP settings from the OpenWrt docs

The bridged-AP guide asks for a static management address on the LAN bridge with the router as gateway and DNS, DHCP server ignored on every interface, IPv6 RA and DHCPv6 services disabled, and says to leave dnsmasq and the firewall running because disabling them saves nothing.[^owrt-bridgedap] Applied here:
