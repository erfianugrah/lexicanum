...
Three ways out, and this guide implements none of them: keep the hub on the critical
path deliberately and give it the availability that implies; put issuance in a real
external IdP so both tiers are verifiers; or migrate the user rows at promotion time
and accept one re-login in exchange for actual independence.
...
- **Recovering from a lost hub.** The coupling is measured above; the mitigations are
  not built or tested. Migrating `auth.users` between projects in particular is
