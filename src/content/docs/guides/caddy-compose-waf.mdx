...
The final stage also installs **two cron jobs**: the IPsum blocklist update (daily at 02:00)
and the Coraza audit log rotation (hourly, copytruncate when >256MB).
...
dashboard REST API in roughly 8,100 lines of Go (plus 9,900 lines of tests across 13 test files).

### File Organization

| File | Responsibility |
|------|---------------|
| `main.go` | HTTP server, 31 route handlers, CORS middleware |
...
| `cli.go` | CLI subcommands (config, rules, events, deploy, blocklist) |
...
exclusions, rate limit zones). This means a full stack restart always picks up the latest
...
format is `<project-version>-<caddy-version>` (e.g., `1.30.0-2.11.1`) and wafctl uses simple
semver (e.g., `0.22.0`). Version is injected at build time via `-ldflags "-X main.version=..."`.
:::
...
```bash
cosign verify your-registry/caddy:1.30.0-2.11.1 \
