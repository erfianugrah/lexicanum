...
wafctl -> volumes.logs: reads (tail)
wafctl -> volumes.coraza: writes configs
wafctl -> volumes.rl: writes zones
caddy -> volumes.coraza: reads configs
caddy -> volumes.rl: reads zones
dashboard -> wafctl: REST API
wafctl -> caddy: admin API reload
```
