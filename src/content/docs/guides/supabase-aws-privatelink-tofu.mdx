...
description: Build the full private path from your VPC to a Supabase project with OpenTofu - project and settings via the supabase provider, the one dashboard click that is not automatable (and why), RAM share acceptance, a Resource-type VPC endpoint serving direct Postgres 5432 and PgBouncer 6543, a Route53 private hosted zone that keeps sslmode=verify-full working, CLI migrations over the endpoint, and the network-restriction lockout - each step independently verifiable.
author: Erfi Anugrah
---
...
Each with the symptom you will actually see. The [reference](/reference/supabase-aws-privatelink/#gotchas) has the full failure-mode analysis per item.
