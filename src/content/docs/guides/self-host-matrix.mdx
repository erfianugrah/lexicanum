...
3. **Jitsi video conferencing**: Prosody, Jicofo, the Jitsi Videobridge (JVB), with Spectrum for media UDP
...
digraph {
  ...
  subgraph cluster_internet {
    label="Internet"; style=rounded; fontsize=12; margin=12;
    internet_clients [label="Element Web/X\nMobile Clients"];
  }
  subgraph cluster_cloudflare {
    label="Cloudflare Edge"; style=rounded; fontsize=12; margin=12;
    cloudflare_proxy    [label="CF Proxy\n(HTTPS)"];
    cloudflare_tunnel   [label="cfd Tunnel\n(HTTP/2)"];
    cloudflare_spectrum [label="Spectrum\n(L4 TCP/UDP)"];
  }
  subgraph cluster_vyos {
    label="VyOS Router"; style=rounded; fontsize=12; margin=12;
    vyos_pppoe0 [label="pppoe0\n(WAN)"];
    vyos_nat    [label="DNAT Rules\n40-45"];
    vyos_fw     [label="EXTERNAL-IN\n(default: drop)"];
    vyos_eth1   [label="eth1\n(LAN 10.0.0.x)"];
  }
  subgraph cluster_k3s {
    label="k3s Cluster (4x ARM64 Rock)"; style=rounded; fontsize=12; margin=12;
    k3s_traefik [label="Traefik LB\n10.0.0.100\n:443 :80 :10000/udp"];
    k3s_metallb [label="MetalLB LB\n10.0.0.102\n(LiveKit media+TURN)"];

    subgraph cluster_k3s_matrix {
      label="matrix namespace"; style=rounded; fontsize=12; margin=12;
      k3s_matrix_synapse  [label="Synapse\n:8008"];
      k3s_matrix_postgres [label="PostgreSQL\n:5432"];
      k3s_matrix_element  [label="Element Web\n:80"];
      k3s_matrix_admin    [label="Synapse Admin\n:8080"];
    }
    subgraph cluster_k3s_jitsi {
      label="jitsi namespace"; style=rounded; fontsize=12; margin=12;
      k3s_jitsi_prosody [label="Prosody XMPP\n:5222/:5280"];
      k3s_jitsi_jicofo  [label="Jicofo"];
      k3s_jitsi_jvb     [label="JVB\n:10000/udp"];
      k3s_jitsi_web     [label="Jitsi Web\n:80"];
    }
    subgraph cluster_k3s_livekit_ns {
      label="livekit namespace"; style=rounded; fontsize=12; margin=12;
      k3s_livekit_ns_livekit [label="LiveKit Server\n:7880/:7882/:5349"];
      k3s_livekit_ns_jwt     [label="lk-jwt-service\n:8080"];
      k3s_livekit_ns_redis   [label="Redis\n:6379"];
    }
  }

  internet_clients -> cloudflare_proxy    [label="HTTPS"];
  internet_clients -> cloudflare_spectrum [label="UDP/TCP\nmedia"];
  cloudflare_proxy -> cloudflare_tunnel;
  cloudflare_tunnel -> vyos_pppoe0        [label="HTTP/2\nencapsulated"];
  cloudflare_spectrum -> vyos_pppoe0      [label="direct-to-IP"];
  vyos_pppoe0 -> vyos_nat;
  vyos_nat -> vyos_fw;
  vyos_fw -> vyos_eth1;
  vyos_eth1 -> k3s_traefik [label="HTTPS + JVB UDP"];
  vyos_eth1 -> k3s_metallb [label="7882, 7881\n5349, 3478\n49160-49200"];
  k3s_traefik -> k3s_matrix_synapse     [label="matrix.example.com"];
  k3s_traefik -> k3s_matrix_element     [label="chat.example.com"];
  k3s_traefik -> k3s_matrix_admin       [label="admin.matrix.example.com"];
  k3s_traefik -> k3s_jitsi_web          [label="jitsi.example.com"];
  k3s_traefik -> k3s_livekit_ns_jwt     [label="matrix-rtc.example.com\n/livekit/jwt"];
  k3s_traefik -> k3s_livekit_ns_livekit [label="matrix-rtc.example.com\n/livekit/sfu"];
  k3s_metallb -> k3s_livekit_ns_livekit [label="media+TURN"];
  k3s_matrix_synapse -> k3s_matrix_postgres;
  k3s_livekit_ns_jwt -> k3s_matrix_synapse [label="OpenID\nvalidation"];
  k3s_livekit_ns_livekit -> k3s_livekit_ns_redis;
  k3s_jitsi_jicofo -> k3s_jitsi_prosody;
  k3s_jitsi_jvb -> k3s_jitsi_prosody;
}
```
...
:::note
The `--lc-collate=C --lc-ctype=C` initdb args are required by Synapse for correct Unicode sorting behaviour.
:::
...
`ENABLE_P2P: "false"` forces all traffic through JVB, so behaviour through Cloudflare Spectrum stays consistent.
...
digraph {
  ...
  client   [label="Element Web"];
  synapse  [label="Synapse\n(matrix.example.com)"];
  jwt      [label="lk-jwt-service\n(matrix-rtc.example.com)"];
  livekit  [label="LiveKit Server"];
  spectrum [label="Spectrum Edge"];

  client -> synapse   [label="1. Get OpenID token", style=dashed];
  client -> jwt       [label="2. POST /livekit/jwt/get_token\nwith OpenID token", style=dashed];
  jwt -> synapse      [label="3. Validate token\nvia federation endpoint", style=dashed];
  jwt -> client       [label="4. Return LiveKit JWT\n+ WSS URL", style=dashed];
  client -> livekit   [label="5. WebSocket signaling\n(via Tunnel -> Traefik)", style=dashed];
  client -> spectrum  [label="6. WebRTC media\n(UDP 7882)"];
  spectrum -> livekit [label="Spectrum -> VyOS\n-> MetalLB -> pod"];
}
```
...
digraph {
  ...
  subgraph cluster_wan {
    label="pppoe0 (WAN)"; style=rounded; fontsize=12; margin=12;
    wan_spectrum [label="Spectrum traffic\nUDP/TCP media"];
    wan_tunnel   [label="Tunnel traffic\nHTTP/2 encap"];
  }
  subgraph cluster_prerouting {
    label="DNAT\n(prerouting)"; style=rounded; fontsize=12; margin=12;
    prerouting_r40 [label="40: 3478/udp -> .102"];
    prerouting_r41 [label="41: 3478/tcp -> .102"];
    prerouting_r42 [label="42: 5349/tcp -> .102"];
    prerouting_r43 [label="43: 49160-49200/udp -> .102"];
    prerouting_r44 [label="44: 7882/udp -> .102"];
    prerouting_r45 [label="45: 7881/tcp -> .102"];
  }
  subgraph cluster_forward {
    label="forward filter\n(default-action: drop)"; style=rounded; fontsize=12; margin=12;
    forward_r5  [label="rule 5: pppoe0\njump EXTERNAL-IN"];
    forward_r14 [label="rule 14: eth1.200\njump TPI"];
  }
  subgraph cluster_external_in {
    label="EXTERNAL-IN\n(default-action: drop)"; style=rounded; fontsize=12; margin=12;
    external_in_r70  [label="70: TURN 3478/udp\nsrc: cf-ipv4"];
    external_in_r71  [label="71: TURN 3478/tcp\nsrc: cf-ipv4"];
    external_in_r72  [label="72: TURNS 5349\nsrc: cf-ipv4"];
    external_in_r73  [label="73: Relay 49160-49200\nsrc: cf-ipv4"];
    external_in_r74  [label="74: LiveKit 7882/udp\nsrc: cf-ipv4"];
    external_in_r75  [label="75: LiveKit 7881/tcp\nsrc: cf-ipv4"];
    external_in_drop [label="default: DROP"];
  }
  subgraph cluster_lan {
    label="eth1.200\n(VLAN 200 - 10.0.0.0/24)"; style=rounded; fontsize=12; margin=12;
    lan_traefik [label="Traefik LB\n10.0.0.100\n:443"];
    lan_metallb [label="MetalLB LB\n10.0.0.102\nmedia+TURN"];
  }

  snat [label="SNAT rule 20\nmasquerade\n10.0.0.0/8 -> pppoe0"];

  wan_spectrum -> prerouting_r40 [label="direct-to-IP", lhead=cluster_prerouting];
  wan_tunnel -> lan_traefik      [label="HTTP/2 -> :443", style=dashed];
  prerouting_r44 -> forward_r5   [label="dst rewritten", lhead=cluster_forward];
  prerouting_r45 -> forward_r5   [lhead=cluster_forward];
  forward_r5 -> external_in_r70  [label="jump", lhead=cluster_external_in];
  external_in_r74 -> lan_metallb [label="accept"];
  external_in_r75 -> lan_metallb [label="accept"];
  external_in_r70 -> lan_metallb [label="accept"];
  external_in_drop -> external_in_drop [label="all else dropped", style=dashed];
  lan_traefik -> snat [label="outbound replies", style=dashed];
  lan_metallb -> snat [label="outbound replies", style=dashed];
  snat -> wan_spectrum [style=dashed, lhead=cluster_wan];
}
```
...
After the initial deployment is working, add security contexts, health probes, and startup ordering to all workloads. The hardening below applies to every component in the Matrix, Jitsi, LiveKit, and Maubot stacks. Maubot is not deployed in this guide; its rows apply only if you run it in its own `maubot` namespace alongside the Matrix stack.
...
| LiveKit `room.auto_create: false`   | Prevents unauthorised room creation                                            |
