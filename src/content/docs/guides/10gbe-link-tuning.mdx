...
| 40G "headroom" | irrelevant to the cap | USB4's 40Gbps is line bandwidth; the constraint is per-transaction latency. |
...
The single-flow remainder (~85% of the ~9.9 jumbo goodput ceiling) is the tunnel DMA-read latency itself. The only fix for that remainder is a native-PCIe NIC; everything software-side is exhausted by the end of this guide. 9.7 Gbps multi-flow exceeds the classic "9.4 line rate" figure because that figure assumes 1500-byte frames; at 9014 the L2 overhead fraction shrinks and the goodput ceiling rises to ~9.9.
...
Validate over an **overnight idle** window: RTD3 arms when the link is quiet, so a load soak never exercises it. On this rig: zero 9003 events after the cable swap + power settings, including idle.
...
### Localise loss before tuning

During retransmit-heavy TX runs, take deltas on every node in the path: router NIC (`ethtool -S`, watch `rx_missed_errors`), switch port counters (RX/TX discards, FCS), Windows NIC discard counters. On this rig every external node was provably clean; the "loss" was **spurious retransmits** - tunnel completion/ACK-return jitter makes Windows TCP declare loss on merely-late packets and clamp cwnd. That is the sawtooth. The fix is to change what the sender pays per packet (Part 4) and how it reacts to jitter (CTCP, below); more bandwidth does not lift the clamp.

When a node is *not* clean, the counter that moved tells you which class of problem you have. Reading the counters is the cheapest diagnostic in this guide; check them before swapping cables.
...
The two classes do not overlap. Corrupted copper cannot produce clean CRCs, so an all-zero physical column exonerates every cable and optic in the path in one command. Link flap count is the companion check - `cat /sys/class/net/<if>/carrier_changes` on Linux, and expect exactly 2 per boot (one down, one up).

A port number in a switch's own flap history is not a machine identity. A port that logged hundreds of link events last month may have had a different host, with a different NIC, plugged into it. Match on the NIC, not the port label, before treating old flap data as evidence about the machine there now.
...
The aggregate barely moves because it was already at the framing ceiling. The rings raise the single-stream floor by 0.82 Gbps and narrow the run-to-run spread by 5.25x; headline throughput stays where it was.

This also disproved the obvious first hypothesis. Before the change, NET_RX softirq during a single-stream run was 46% concentrated on one core, which reads like RSS single-queue saturation. After the change that concentration is essentially unchanged (36%) while throughput is stable - one core was always sufficient, it just needed descriptors.
...
802.3x pause frames do not fit a **trunk**: a pause frame is link-global with no VLAN awareness, so pausing one host's bulk transfer stalls every VLAN sharing that trunk, trading a buffer drop on one segment for head-of-line blocking across five. The per-priority mechanism that would do this correctly is PFC (802.1Qbb), which small managed switches generally do not implement.
...
NIC advanced property. EEE on this link produced a 1.3-2.9 Gbps sawtooth; disabling it stabilised the intervals at a steady ~2.7-2.9. It removes noise without raising the cap, and it is a known AQtion instability source, so keep it off regardless.[^archimago-aqc]
...
4.33 -> 5.38 Gbps (+24%). Batching completions (Extreme/High moderation) adds delay per completion, and on this path throughput is bounded by completion latency. This rig first tested the wrong direction (Adaptive -> Extreme): interrupts dropped 90k -> 23k/s and throughput did not move, because interrupt rate was never the cap. Disable moderation outright; CPU headroom on any modern box absorbs it.
...
:::caution
Never install per-packet shapers (ROG GameFirst, cFosSpeed-class filter drivers) on this box. Windows Filtering Platform (WFP)/filter drivers sit in the TX path and do per-packet processing - exactly the cost this whole guide is minimising. Check what is bound with `Get-NetAdapterBinding`.
:::
...
:::caution
VLAN children inherit the parent MTU, so without the `or 1500` default, raising a trunk to 9014 flips **every** VLAN to jumbo - and then big frames toward 1500-MTU hosts (servers, IoT) black-hole, because UDP has no MSS negotiation. Jumbo belongs on exactly the one segment whose hosts you control.
:::
...
Declaring `MTUBytes`/`mtu` only when you want jumbo makes reverting it later fail silently. Both systemd-networkd and the NixOS activation manage MTU **only when the option is present**: deleting the line stops managing the value instead of restoring 1500, and the last-set MTU persists on the live interface across reboots and rebuilds.

The symptom is a successful, apparently-clean deploy that changes nothing. Reverting jumbo on the rig above left the trunk at 9014 and a server NIC at 9000 through a green `nixos-rebuild switch`. Both needed a manual `ip link set dev <iface> mtu 1500` to land, and until then the config said 1500 while the live interfaces did not.
...
Pin the number on every managed link, jumbo or not, so that removing jumbo is an edit from 9014 to 1500 that the next switch applies.
...
- **Check which host controller you have before downloading vendor tools.** This AM5 board's USB4 is Intel Maple Ridge (`VEN_8086&DEV_1137`), not the ASMedia ASM4242 that most AM5 boards and the board vendor's own FAQ assume - the ASM4242 firmware tools error out with "Can't find any ASM4242 controller".
