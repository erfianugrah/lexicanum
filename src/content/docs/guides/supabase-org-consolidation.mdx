...
| Secret keys can be bound to a role via `secret_jwt_template` | `POST /v1/projects/{ref}/api-keys` with a template on a platform-plan organization, 2026-08-24 | Measured - accepted (`201`) and echoed; the minted key is opaque (`sb_secret_...`), so the template shapes the token server-side |
