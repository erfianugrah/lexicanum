...
group: connectivity
featured: true
blurb: "Interop with the IPsec/GRE estate, MTU/MSS arithmetic included."
---

{/* prose-dollar: math-intentional - MTU/MSS arithmetic is rendered as LaTeX */}
...
To avoid performance issues caused by packet fragmentation, calculate the Maximum Segment Size (MSS) from the Maximum Transmission Unit (MTU) and any encapsulation overhead.
...
The MSS value is advertised during the three-way TCP handshake. Use `tcpdump` to capture the initial SYN packets and inspect the MSS option.
...
### Step 4: make sure [health-checks](https://developers.cloudflare.com/cloudflare-wan/reference/tunnel-health-checks/) work
...
digraph {
  ...
  subgraph cluster_Cloudflare {
    ...
    Cloudflare_CF1 [label="Cloudflare"];
    Cloudflare_CF2 [label="Cloudflare"];
  }
  subgraph cluster_Users {
    label="Users"; style=rounded; fontsize=12; margin=12;
    Users_PAC      [label="PAC File User"];
    Users_RBI      [label="Clientless RBI User"];
    Users_MWUser   [label="Magic WAN User"];
    Users_WARPUser [label="WARP User"];
  }
  subgraph cluster_WARPConn {
    label="WARPConn"; style=rounded; fontsize=12; margin=12;
    WARPConn_Connector [label="WARP Connector"];
    WARPConn_VOIP      [label="Private Service - VOIP"];
  }
  subgraph cluster_MagicWAN {
    label="MagicWAN"; style=rounded; fontsize=12; margin=12;
    MagicWAN_MW         [label="Magic WAN"];
    MagicWAN_FileServer [label="Private Service - File Server"];
  }
  subgraph cluster_MWConnector {
    label="MWConnector"; style=rounded; fontsize=12; margin=12;
    MWConnector_Conn [label="Magic WAN Connector"];
    MWConnector_SSH  [label="SSH Server"];
  }
  subgraph cluster_CFTunnel {
    label="CFTunnel"; style=rounded; fontsize=12; margin=12;
    CFTunnel_Tunnel [label="Cloudflare Tunnel"];
    CFTunnel_CRM    [label="Private Service - CRM"];
  }

  Users_PAC -> Cloudflare_CF1      [label="HTTP"];
  Users_PAC -> Cloudflare_CF2      [label="HTTP"];
  Users_RBI -> Cloudflare_CF1      [label="HTTP"];
  Users_RBI -> Cloudflare_CF2      [label="HTTP"];
  Users_MWUser -> Cloudflare_CF1   [label="GRE or IPsec"];
  Users_WARPUser -> Cloudflare_CF1 [label="QUIC or Wireguard"];
  Users_WARPUser -> Cloudflare_CF2 [label="QUIC or Wireguard"];

  Cloudflare_CF2 -> WARPConn_Connector [label="QUIC or Wireguard"];
  WARPConn_Connector -> WARPConn_VOIP  [dir=both];

  Cloudflare_CF1 -> MagicWAN_MW        [label="GRE or IPsec"];
  MagicWAN_MW -> MagicWAN_FileServer   [dir=both];

  Cloudflare_CF1 -> MWConnector_Conn   [label="GRE or IPsec"];
  MWConnector_Conn -> MWConnector_SSH  [dir=both];

  Cloudflare_CF1 -> CFTunnel_Tunnel    [label="QUIC or H2"];
  CFTunnel_Tunnel -> CFTunnel_CRM;
}
```
...
There are many ways to deploy the PAC file such as MDMs and using a remote server, in this case, I'll be using [Workers](https://github.com/erfianugrah/worker-proxy-pac). You can follow the steps [here](https://developers.cloudflare.com/cloudflare-one/team-and-resources/devices/agentless/pac-files/), when using the code, you can create more cases to match specific files, in my case it is nl.pac and sg.pac to simulate the two locations, I also used [Workers Secrets](https://developers.cloudflare.com/workers/configuration/secrets/) to store the Proxy Endpoint domains:
...
#### WARP/Clientless RBI (remote browser isolation)
...
- Enable CGNAT (carrier-grade NAT) routing (Settings -> Network)
...
#### Spectrum / Load Balancer to Private Origins (Magic WAN)

[Private Network Load Balancing](https://developers.cloudflare.com/load-balancing/private-network/) with Magic WAN off-ramps traffic to private IPs behind your IPsec/GRE tunnels. For HTTP, a proxied LB pointing to private origins on the default VNET is sufficient. For TCP/UDP, you need [Spectrum](https://developers.cloudflare.com/spectrum/) in front of the LB since raw L4 traffic can't go through the HTTP proxy, refer to the [Spectrum LB docs](https://developers.cloudflare.com/load-balancing/additional-options/spectrum/) for more.

The pool origins need to be on the **default VNET** - this is the account-level VNET where Magic WAN tunnels live, not a Cloudflare Tunnel VNET.
...
digraph {
  ...
  subgraph cluster_Users {
    label="Users"; style=rounded; fontsize=12; margin=12;
    Users_HTTP [label="HTTP Client"];
    Users_L4   [label="TCP/UDP Client"];
  }
  subgraph cluster_Cloudflare {
    label="Cloudflare"; style=rounded; fontsize=12; margin=12;
    Cloudflare_Proxy    [label="Cloudflare Proxy"];
    Cloudflare_Spectrum [label="Spectrum"];
    Cloudflare_LB1      [label="Load Balancer"];
    Cloudflare_LB2      [label="Load Balancer"];
  }
  subgraph cluster_MagicWAN {
    label="MagicWAN"; style=rounded; fontsize=12; margin=12;
    MagicWAN_MW     [label="Static Route"];
    MagicWAN_Tunnel [label="IPsec / GRE Tunnel"];
  }
  subgraph cluster_Origin {
    label="Origin"; style=rounded; fontsize=12; margin=12;
    Origin_Router  [label="On-prem Router"];
    Origin_HTTPSvc [label="HTTP Service"];
    Origin_L4Svc   [label="TCP/UDP Service"];
  }

  Users_HTTP -> Cloudflare_Proxy        [label="HTTPS"];
  Cloudflare_Proxy -> Cloudflare_LB1;
  Cloudflare_LB1 -> MagicWAN_MW         [label="private IP (default VNET)"];
  MagicWAN_MW -> MagicWAN_Tunnel        [label="nexthop"];
  MagicWAN_Tunnel -> Origin_Router      [label="GRE or IPsec"];
  Origin_Router -> Origin_HTTPSvc;

  Users_L4 -> Cloudflare_Spectrum       [label="TCP or UDP"];
  Cloudflare_Spectrum -> Cloudflare_LB2 [label="origin_dns"];
  Cloudflare_LB2 -> MagicWAN_MW         [label="private IP (default VNET)"];
  MagicWAN_Tunnel -> Origin_Router;
  Origin_Router -> Origin_L4Svc;
}
```
...
- Spectrum LBs are always proxied regardless of what you set on the LB itself
- No session affinity support (except hash endpoint steering), no Custom Rules
- The LB hostname and the Spectrum application DNS name must be different or you'll get DNS resolution conflicts
- For UDP origins, use `icmp_ping` monitors - HTTP monitors won't work against a UDP service
- For HTTP origins over private networks, use `http` (not `https`) monitors unless the origin terminates TLS
:::

##### HTTP: LB to Magic WAN

For HTTP, the LB is just a standard proxied LB with pool origins pointing at private IPs on the default VNET. The traffic gets off-ramped via the Magic WAN static route to the IPsec/GRE tunnel, same as any other Magic WAN traffic.
...
resource "cloudflare_load_balancer_pool" "httpbun_ipsec_erfipie_nl" {
  ...
  origins {
    ...
    virtual_network_id = var.default_vnet_id  # default VNET (Magic WAN)
    name               = "httpbun_ipsec_erfipie_nl"
    weight             = 1
  }
}
...
##### TCP/UDP: Spectrum to LB to Magic WAN

For TCP/UDP, you first create the LB and pool the same way as the HTTP case, then create a Spectrum application that uses `origin_dns` to point at the LB hostname. The Spectrum app handles the L4 on-ramp, the LB steers to the pool, and Magic WAN off-ramps to your tunnel.

The LB hostname and Spectrum DNS name need to be different - in my case, `jitsi-lb.example.com` for the LB and `jitsi-udp.example.com` for the Spectrum entry point.

##### Gitops/IaC

###### LB Monitor HCL

Since the origin is a UDP service (Jitsi JVB), I'm using `icmp_ping` instead of an HTTP health check:
...
:::note
For TCP origins, you can use a `tcp` monitor instead of `icmp_ping`.
:::
...
###### Spectrum Application HCL

The Spectrum application uses `origin_dns` (not `origin_direct`) to reference the LB hostname, `traffic_type` is `direct` for raw L4 passthrough:
