...
%%{init: { 
  ...
  'themeVariables': {
     ...
     'lineColor': '#F6821F',
     'secondaryColor': '#FFCF96',
     'tertiaryColor': '#fff'
  },
  'darkMode': true,
  'themeCSS': '.label { color: #333; } .cluster rect, .node rect, .node circle, .node ellipse, .node polygon, .node path { fill: #f6821f; } .dark-mode .label { color: #eee; } .dark-mode .cluster rect, .dark-mode .node rect, .dark-mode .node circle, .dark-mode .node ellipse, .dark-mode .node polygon, .dark-mode .node path { fill: #F6821F; }'
}}%%
graph LR;
  classDef cloudflare fill: %%primaryColor%%, stroke: %%primaryBorderColor%%, stroke-width:2px, color: %%primaryTextColor%%;
  classDef service fill:#e8e8e8,stroke:#666,color:#333;
  classDef serviceUser fill:#e8eef7,stroke:#5a7abf,color:#333;
  ...
  subgraph Users
    M[PAC File User]:::serviceUser --> |HTTP| K
    M --> |HTTP| L
    N[Clientless RBI User]:::serviceUser --> |HTTP| K
    N --> |HTTP| L
    F[Magic WAN User]:::serviceUser --> |GRE or IPsec| K
    A[WARP User]:::serviceUser --> |QUIC or Wireguard| K
    A --> |QUIC or Wireguard| L
  end
  
  subgraph WARP Connector   
    L -->|QUIC or Wireguard| H[WARP Connector] <--> I[Private Service - VOIP]:::service   
  end
  
  subgraph Magic WAN
    K --> |GRE or IPsec| D[Magic WAN] <--> E[Private Service - File Server]:::service 
  end
  
  subgraph Magic WAN Connector
    K --> |GRE or IPsec| G[Magic WAN Connector] <--> J[SSH Server]:::service
  end
  
  subgraph Cloudflare Tunnel
    K --> |QUIC or H2| B[Cloudflare Tunnel] --> C[Private Service - CRM]:::service 
  end
```
