...
The overhead for IPsec's Encapsulating Security Payload (ESP) is variable and depends on the encryption and integrity algorithms used. It includes a new IP header, ESP header, Initialization Vector (IV), and trailer/authenticator.[9, 10]

A conservative estimate for a common suite like **AES-CBC with SHA-256** can be up to **77 bytes**.[9] More efficient modern ciphers like **AES-GCM** have a fixed overhead around **54 bytes**.

| Cipher Suite             | Mode   | New IP Header (Bytes) | ESP Header (Bytes) | IV Size (Bytes) | Max Padding (Bytes) | Trailer (Bytes) | ICV (Bytes) | Total Max Overhead (Bytes) |
| :----------------------- | :----- | :-------------------- | :----------------- | :-------------- | :------------------ | :-------------- | :---------- | :------------------------- |
| AES-256-CBC with SHA-256 | Tunnel | 20                    | 8                  | 16              | 15                  | 2               | 16          | **77**                     |
| AES-256-GCM              | Tunnel | 20                    | 8                  | 8               | N/A                 | 2               | 16          | **54**                     |
| 3DES-CBC with SHA-1      | Tunnel | 20                    | 8                  | 8               | 7                   | 2               | 12          | **57**                     |
...
```sh
ping <DESTINATION_IP> -M do -s <payload_size>
