...
digraph {
  ...
  subgraph cluster_pages {
    label="Cloudflare Pages"; style=rounded; fontsize=12; margin=12;
    pages_features [label="- `wrangler pages deploy`\n- Simpler setup\n- Auto 404/SPA handling\n- Preview deployments\n- pages.dev subdomain", shape=plaintext];
  }
  subgraph cluster_workers {
    label="Workers Static Assets"; style=rounded; fontsize=12; margin=12;
    workers_features [label="- `wrangler deploy`\n- wrangler.jsonc config\n- Durable Objects\n- Cron Triggers\n- Logpush\n- Custom routes\n- Preview (version) URLs", shape=plaintext];
  }

  pages_features -> workers_features [label="migrate when\nyou need more features", style=dashed, ltail=cluster_pages, lhead=cluster_workers];
}
```
...
| Preview deployments | ✅ | ✅ (version URLs) |
| Durable Objects | ❌ | ✅ |
| Cron Triggers | ❌ | ✅ |
| Logpush | ❌ | ✅ |
| Tail Workers | ❌ | ✅ |
...
digraph {
  ...
  Push_PR [label="Push/PR", shape=ellipse];
  Setup   [label="Bun + D2 + Caches"];
  Build   [label="bun run build"];
  Deploy  [label="wrangler pages deploy", shape=hexagon];

  Push_PR -> Setup -> Build -> Deploy;
}
```
...
```
A new run cancels any in-progress deployment, so only one runs at a time.
...
- name: Cache Bun dependencies
  ...
  with:
    ...
    restore-keys: |
      ...
      ${{ runner.os }}-bun-
```
- Caches `node_modules` and Bun's install cache; the key includes the OS, the lockfile hash and the package.json hash, so it invalidates when dependencies change
...
- name: Cache D2 CLI
  ...
  with:
    ...
    restore-keys: |
      ${{ runner.os }}-d2-
```
- The key carries the D2 version (`v0.7.1`), so bumping the version invalidates the cache
...
- name: Install project dependencies
  run: bun install
```
- Hits the Bun cache from step 3 when the lockfile is unchanged
...
```
- Only runs if the D2 cache missed; D2 is used by the `remark-d2` plugin during the Astro build
...
```
- Fails fast if `d2` is missing from PATH, then retries `bun run build` for transient failures, up to 3 times with 30-second delays
...
- name: Deploy to Cloudflare Pages
  ...
  with:
    ...
    packageManager: bun
```
- Passes the step 7 git values to `wrangler pages deploy` as deployment metadata; `wranglerVersion` pins Wrangler for reproducibility
...
digraph {
  ...
  Push   [label="Push", shape=ellipse];
  Build  [label="Build + Upload Artifact"];
  Deploy [label="wrangler deploy", shape=hexagon];
  Purge  [label="Cache Purge", shape=diamond];

  Push -> Build;
  Build -> Deploy;
  Deploy -> Purge [label="proxied zone"];
}
```
