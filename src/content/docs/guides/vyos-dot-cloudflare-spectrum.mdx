---
title: VyOS DoT with Magic WAN
description: End-to-end setup of a DNS-over-TLS endpoint on VyOS 1.4 using Glory-Hole, DNAT, policy routing, and Magic WAN IPsec for hairpin traffic.
author: "Erfi Anugrah"
---

Expose a Glory‑Hole DoT listener on VyOS 1.4 via DNAT (853 → 10.0.10.10:853) directly to the internet. LAN hosts hairpin through Magic WAN IPsec. All Cloudflare traffic routes via Magic WAN since we can't differentiate site-local Cloudflare services from internet Cloudflare.
...
%%{init: {
  ...
  "themeCSS": ".mermaid { background-color: #0b1220 !important; } .edgePath path, .messageLine0, .messageLine1, marker path { stroke: #ffffff !important; fill: #ffffff !important; } .edgeLabel, .edgeLabel rect, .edgeLabel span { color: #ffffff !important; background-color: #0b1220 !important; } .nodeLabel { color:#ffffff !important; fill:#ffffff !important; }"
}}%%
flowchart TB
    classDef wan fill:#1f2937,stroke:#0ea5e9,stroke-width:2px,color:#f8fafc
    classDef svc fill:#1d4ed8,stroke:#3b82f6,stroke-width:2px,color:#f8fafc
    classDef mw fill:#0f766e,stroke:#14b8a6,stroke-width:2px,color:#f8fafc
    classDef lan fill:#312e81,stroke:#7c3aed,stroke-width:2px,color:#f8fafc

    Internet[Internet Clients]:::lan -->|DoT port 853| WAN[VyOS WAN<br/>pppoe0<br/>203.0.113.x]:::wan

    WAN -->|DNAT rule 29<br/>853→10.0.10.10:853| Container[Glory-Hole Container<br/>10.0.10.10:853<br/>pod-podman-2]:::svc

    LAN[LAN Hosts<br/>10.0.69.x<br/>10.0.0.x]:::lan -.->|Hairpin via<br/>Magic WAN| MagicWAN[Magic WAN<br/>IPsec vti0]:::mw
    MagicWAN -.->|table 20| Container

    Container -->|To Cloudflare IPs<br/>rule 1 → table 20| MagicWAN
    Container -->|Catch-all rule 2<br/>→ main table| WAN
...
set policy route magic-wan-ipsec-glory-hole rule 6 destination address '10.0.0.100'
...
%%{init: {
  ...
  "themeCSS": ".mermaid { background-color: #0b1220 !important; } .edgePath path, marker path { stroke: #ffffff !important; fill: #ffffff !important; } .edgeLabel, .edgeLabel rect, .edgeLabel span { color: #ffffff !important; background-color: #0b1220 !important; } .nodeLabel { color:#ffffff !important; fill:#ffffff !important; }"
}}%%
flowchart TD
    classDef main fill:#1f2937,stroke:#0ea5e9,stroke-width:2px,color:#f8fafc
    classDef alt fill:#0f766e,stroke:#14b8a6,stroke-width:2px,color:#f8fafc
    classDef decision fill:#1d4ed8,stroke:#3b82f6,stroke-width:2px,color:#f8fafc

    Start["Packet from Glory-Hole<br/>10.0.10.10"]:::decision --> Rule1{Rule 1:<br/>cf-ipv4?}:::decision
    Rule1 -->|Yes| Table20A["table 20<br/>(vti0 IPsec)"]:::alt
    Rule1 -->|No| Rule2{Rule 2:<br/>catch-all to main}:::decision
    Rule2 -->|Matches| Main1["main table<br/>(pppoe0)"]:::main
    Rule2 -->|If narrowed| Hairpin{Rules 5-8:<br/>LAN hairpins}:::decision
    Hairpin -->|Yes| Table20B["table 20<br/>(vti0 IPsec)"]:::alt
    Hairpin -->|No| Rule100["Rule 100:<br/>catch-all"]:::decision
    Rule100 --> Main2["main table<br/>(pppoe0)"]:::main

    class Start,Rule1,Rule2,Hairpin,Rule100 decision
    class Table20A,Table20B alt
...
set policy route magic-wan-ipsec-traefik rule 5 source address '10.0.0.100'
set policy route magic-wan-ipsec-traefik rule 100 set table 'main'
set policy route magic-wan-ipsec-traefik rule 100 source address '10.0.0.100'
```
...
```sh
kdig @203.0.113.42 -p 853 +tls-host=your-dot-host +tls-ca A example.com
```
...
sudo tcpdump -i pppoe0 -n host 203.0.113.42 and port 853
```
