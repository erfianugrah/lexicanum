...
6. **Security hardening**: SOPS encryption, firewall source groups, path isolation
   :::
...
| **`LIVEKIT_FULL_ACCESS_HOMESERVERS`** | Defaults to `*` if unset. Always restrict to your homeserver domain.                                 |
...
| lk-jwt-service homeserver allowlist | `LIVEKIT_FULL_ACCESS_HOMESERVERS=matrix.example.com` (not `*`)                 |
