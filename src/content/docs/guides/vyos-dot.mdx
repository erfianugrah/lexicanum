...
category: networking
featured: true
blurb: "Running a DoT service across the WAN and the router."
---
...
digraph {
  ...
  Internet  [label="Internet Clients"];
  WAN       [label="VyOS WAN (pppoe0)"];
  Container [label="Glory-Hole (10.0.10.10:853)"];
  LAN       [label="LAN Hosts"];
  MagicWAN  [label="Magic WAN (vti0)"];

  Internet -> WAN       [label="DoT :853"];
  WAN -> Container      [label="DNAT"];
  LAN -> MagicWAN       [label="hairpin", style=dashed];
  MagicWAN -> Container [label="table 20", style=dashed];
  Container -> MagicWAN [label="CF IPs -> table 20"];
  Container -> WAN      [label="catch-all -> main"];
}
```
...
digraph {
  ...
  Start   [label="Packet from 10.0.10.10", shape=diamond];
  Rule1   [label="cf-ipv4?", shape=diamond];
  Table20 [label="table 20 (vti0)"];
  Rule2   [label="catch-all", shape=diamond];
  Main    [label="main (pppoe0)"];

  Start -> Rule1;
  Rule1 -> Table20 [label="Yes"];
