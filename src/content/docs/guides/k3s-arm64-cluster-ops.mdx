...
category: kubernetes
featured: true
blurb: "Node management and recovery on a Turing Pi cluster."
---
...
6. **Disk maintenance**: Journal size limits, containerd log rotation, NFS storage for the Loki time-series database (TSDB)
...
| :---- | :---- | :---- | :---- | :---- |
| rock1 | 10.0.0.9 | control-plane (server) | 1 | API server, etcd, scheduler |
| rock2 | 10.0.0.10 | agent | 2 | |
| rock3 | 10.0.0.11 | agent | 3 | NFS server (~460 GB at `/data`) |
| rock4 | 10.0.0.12 | agent | 4 | |
...
digraph {
  ...
  subgraph cluster_tpi {
    label="Turing Pi 2 Board\n(BMC: 10.0.0.8)"; style=rounded; fontsize=12; margin=12;
    tpi_slot1 [label="Slot 1\nrock1 (10.0.0.9)\nk3s server", style=dashed];
    tpi_slot2 [label="Slot 2\nrock2 (10.0.0.10)\nk3s agent"];
    tpi_slot3 [label="Slot 3\nrock3 (10.0.0.11)\nk3s agent + NFS"];
    tpi_slot4 [label="Slot 4\nrock4 (10.0.0.12)\nk3s agent"];
  }

  subgraph cluster_vyos {
    label="VyOS Router\n10.0.0.1"; style=rounded; fontsize=12; margin=12;
    vyos_wan [label="PPPoE WAN"];
    vyos_lan [label="LAN 10.0.0.0/24"];
  }

  cf [label="Cloudflare\nTunnel"];

  vyos_lan -> tpi_slot1 [label="1 Gbps", lhead=cluster_tpi];
  cf       -> vyos_wan  [label="kubectl.example.com\n(CF Access + OIDC)"];
  vyos_wan -> vyos_lan  [label="kubectl.example.com\n(CF Access + OIDC)"];
}
```
...
| File | Purpose |
| :---- | :---- |
| `/boot/firmware/ubuntuEnv.txt` | Kernel cmdline (`bootargs=`), the device tree blob (DTB) file, overlays |
...
NODENAME=$(hostname)
SERVER_URL="https://10.0.0.9:6443"  # Replace with your server IP
