...
1. **Secrets.** Put the new SSID and key in the sops file and in the hub's live `secrets.yaml`. Compare fingerprints across the AP's env file, sops and the hub; the new value should never appear in a terminal. The one exception on the rig was the old key, pasted once to stand up the temporary bridge SSID and retired with it (below). `sops` can be driven with a scripted `$EDITOR` for this.
