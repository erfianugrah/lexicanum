...
Never set `hostname:` on the client service. `network_mode: "service:..."` makes it inherit the VPN container's hostname; setting one explicitly triggers `Error response from daemon: conflicting options: hostname and the network mode` and the container will not start.

`SLSKD_REMOTE_CONFIGURATION=true` is required: with it `false`, the PATCH endpoint returns 403 and the port-forward sync path breaks permanently - the client stays on its default listen port and never gets the VPN's forwarded port.
...
On first start, slskd indexes its shared directories. The log line `Listening for HTTP requests at http://:::5030/` appears near the start of the scan, but Kestrel does not accept TCP connections until the scan completes.[^slskd-1160]

Measured on the servarr NAS in August 2026 (`tank` raidz2 pool): 14 minutes for 77,000 files on HDD storage.
...
The client has no IP of its own and so cannot take a macvlan (see [Arr stack Docker patterns](/guides/arr-stack-docker-patterns/)). Instead the VPN container, which owns the netns, publishes the web UI on the Docker host's LAN IP (`ports: - "10.0.0.2:5030:5030"` on `wg-pia-slskd`, with `LOCAL_NETWORK` widened to admit LAN clients), and the edge Caddy proxies that:
