...
cliamp on WSL2 often plays (progress bar advances, "Streaming" lights up) while producing no sound. WSLg ships a working PulseAudio server over RDP that `paplay` can use out of the box, so WSLg itself is fine. cliamp's Linux audio output uses ALSA, and on a default WSL2 distro there is no ALSA device at all - `/dev/snd` is absent. cliamp then opens its `default` PCM, fails silently at startup, and never retries. The fix is the `pulseaudio-alsa` ALSA PCM plugin, which gives cliamp a `default` PCM that routes into WSLg's Pulse server.
...
| cliamp is a Go app built on Beep + Oto | The Oto release cliamp pins (v3.4.1) outputs on Linux through ALSA only: it opens the ALSA `default` PCM through libasound (`snd_pcm_*`). Newer Oto releases use PulseAudio first and fall back to ALSA. cliamp's own device list comes from `pactl`.[^oto-readme] |
...
| cliamp reads `~/.config/cliamp/config.toml` at startup and writes back on quit | Settings chosen in the TUI (theme, EQ, visualiser, provider) persist; the audio device can also be pinned explicitly (below). |
...
The list should contain a cliamp entry against `RDPSink`, and the sink should be `RUNNING`. Before the fix, the symptom was an empty `sink-inputs` list and a `SUSPENDED` sink while cliamp's UI claimed to be streaming.
...
In the original failure the last two rows showed it: `speaker-test` had no ALSA default to open (no plugin), and `pactl list short sink-inputs` stayed empty while cliamp showed "Streaming".

## Persisting the audio device choice

cliamp writes its state back to `~/.config/cliamp/config.toml` on exit, so settings changed in the TUI (theme via `cliamp theme <name>`, visualiser, EQ curve, repeat/shuffle, volume) survive a restart without any extra step.

Pin the audio device explicitly, because on WSL2 you want cliamp to keep using the Pulse-routed default rather than rediscovering a device each launch. Set it once from the CLI:
...
Manual configuration is four keys in `~/.config/cliamp/config.toml`:
...
- **Do not set `PULSE_SERVER` yourself.** WSLg pre-populates it to `unix:/mnt/wslg/PulseServer`; a stray `export PULSE_SERVER=...` in `.zshrc`/`.bashrc` pointing at a TCP or wrong socket is the usual way to break all WSLg audio.[^wslg-issue-29]
...
[^oto-readme]: ebitengine/oto, v3.4.1 `driver_unix.go` (cgo ALSA, `snd_pcm_open` on `default`); the current README describes the newer PulseAudio-first backend with an ALSA fallback. https://github.com/hajimehoshi/oto
