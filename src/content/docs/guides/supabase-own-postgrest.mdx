...
| Add the PostgREST host's egress IP to the network restriction list, or run the container in the VPC behind PrivateLink, before combining this guide with restrictions; an excluded address is refused with `FATAL (EADDRNOTALLOWED) address not in tenant`. | [S10](https://github.com/erfianugrah/supabase-lab/blob/713c28f/experiments/security-lockdown/tests/s10-socket-lock.ts) |
| Exclude the health-check path from `limit_req` (its own `location` or zone); the readiness poll spent the per-window allowance and the module waited a full window before the burst. | [S12](https://github.com/erfianugrah/supabase-lab/blob/713c28f/experiments/security-lockdown/tests/s12-ratelimit-worker.ts) |
...
| Pin `search_path` on `ip_filter` (`alter function public.ip_filter() set search_path = '';`) and run the Management API security advisor after steps 3-5; it flags `function_search_path_mutable` and caught every seeded exposure. | [S01](https://github.com/erfianugrah/supabase-lab/blob/713c28f/experiments/security-lockdown/tests/s01-security-advisors.ts) |
