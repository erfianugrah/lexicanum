...
For an allowlist instead of a ban, invert the test (`raise` unless `xff` is in your set). Read the client IP from `x-forwarded-for` only if the edge in front overwrites it; if the edge appends, a client can spoof it. Pin the function's `search_path` (`alter function public.ip_filter() set search_path = '';`) so the Management API security advisor does not flag it as `function_search_path_mutable`, and run the advisor after steps 3-5: in security-lockdown S01 it caught every seeded exposure. The pinned form was not run through S04.
...
Two adjustments the lab's config did not carry. Exclude any health-check path from `limit_req` (its own `location` or its own zone): in security-lockdown S12, the Worker variant of this limiter, the readiness poll spent the per-window allowance and the module had to wait a full window before the burst. And add `proxy_set_header X-Forwarded-For $remote_addr;` in the `location` block so the step 5 filter sees the peer address rather than a client-supplied header; as written, nginx passes the client's header through. S04 measured the spoof rejection against PostgREST directly; the through-nginx behaviour is not measured.
...
| Add `proxy_set_header X-Forwarded-For $remote_addr;` to the nginx `location` block so the step 5 filter sees the peer address; as written nginx passes a client-supplied header through. S04 measured the spoof rejection at PostgREST directly; the through-nginx behaviour is reasoned, not measured. | S04 |
...
The reproducible form is the supabase-lab [`security-lockdown`](https://github.com/erfianugrah/supabase-lab/tree/0998c69/experiments/security-lockdown) experiment: [`s04-self-hosted-postgrest.ts`](https://github.com/erfianugrah/supabase-lab/blob/0998c69/experiments/security-lockdown/tests/s04-self-hosted-postgrest.ts) (Data API off, container, IP filter), [`s05-rate-limit.ts`](https://github.com/erfianugrah/supabase-lab/blob/0998c69/experiments/security-lockdown/tests/s05-rate-limit.ts) (nginx burst), [`ratelimit.nginx.conf`](https://github.com/erfianugrah/supabase-lab/blob/0998c69/experiments/security-lockdown/ratelimit.nginx.conf), [`s06-postgrest-role.ts`](https://github.com/erfianugrah/supabase-lab/blob/0998c69/experiments/security-lockdown/tests/s06-postgrest-role.ts) (the connection-role check), and the `postgrest-up` / `ratelimit-up` Makefile targets.
