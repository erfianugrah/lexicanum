...
| Expose port 3000 to the edge only. | The filter cannot tell a client-supplied `x-forwarded-for` from an edge-set one; the allowlist property comes from the container's reachability; the function judges whatever header arrives. | [S20](https://github.com/erfianugrah/supabase-lab/blob/713c28f/experiments/security-lockdown/tests/s20-xff-trust-boundary.ts) |
