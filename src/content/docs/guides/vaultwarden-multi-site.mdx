...
category: homelab
featured: true
blurb: "PostgreSQL, R2 sync, and Cloudflare load balancer failover."
---
...
9. **Hardening**: securityContext on all containers, image pinning, CronJob safety
10. **Gotchas and lessons learned**: Volatile storage flag, WebSocket port removal in v1.31.0, tunnel UUID staleness, pgloader quirks, `bash` vs `sh` in Alpine images, restore filename mismatch
:::
...
digraph {
  ...
  subgraph cluster_servarr {
    label="Servarr (Unraid, Site A)"; style=rounded; fontsize=12; margin=12;
    servarr_vw_primary [label="Vaultwarden 1.35.3\n(primary, writes)"];
    servarr_pg_primary [label="PostgreSQL 17\n(172.20.0.4:5432)"];
    servarr_backup     [label="pg_backup_r2\n(every 15 min)"];
    servarr_tunnel_sg  [label="cloudflared\n(servarr tunnel)"];
  }
  subgraph cluster_r2 {
    label="Cloudflare R2"; style=rounded; fontsize=12; margin=12;
    r2_bucket [label="s3://vault/pg-dumps/\n- timestamped tarballs\n- vaultwarden-pg-latest.tar.gz"];
  }
  subgraph cluster_cf {
    label="Cloudflare Edge"; style=rounded; fontsize=12; margin=12;
    cf_lb      [label="Load Balancer\nvault.example.com\n(active/passive)"];
    cf_monitor [label="HTTPS Monitor\nGET /alive :443\n(every 60s)"];
  }
  subgraph cluster_k3s {
    label="k3s Cluster (ARM64, Site B)"; style=rounded; fontsize=12; margin=12;
    k3s_restore    [label="Restore CronJob\n(every 15 min)"];
    k3s_pg_standby [label="PostgreSQL 17\n(ClusterIP :5432)"];
    k3s_vw_standby [label="Vaultwarden 1.35.3\n(standby, read-only)"];
    k3s_tunnel_nl  [label="cloudflared\n(k3s tunnel)"];
    k3s_backup_k3s [label="Backup CronJob\n(daily, for failover)"];
  }

  clients [label="Bitwarden Clients"];

  clients    -> cf_lb             [label="HTTPS"];
  cf_lb      -> servarr_tunnel_sg [label="default pool"];
  cf_lb      -> k3s_tunnel_nl     [label="fallback pool", style=dashed];
  cf_monitor -> servarr_tunnel_sg [label="health check"];
  cf_monitor -> k3s_tunnel_nl     [label="health check"];

  servarr_vw_primary -> servarr_pg_primary [label="DATABASE_URL"];
  servarr_backup     -> servarr_pg_primary [label="pg_dump"];
  servarr_backup     -> r2_bucket          [label="upload"];

  r2_bucket      -> k3s_restore    [label="download"];
  k3s_restore    -> k3s_pg_standby [label="pg_restore"];
  k3s_vw_standby -> k3s_pg_standby [label="DATABASE_URL"];
  k3s_backup_k3s -> k3s_pg_standby [label="pg_dump\n(failover mode)"];
  k3s_backup_k3s -> r2_bucket      [label="upload\n(failover mode)", style=dashed];
}
```
...
2. **main container** (`postgres:17.4-alpine`): Extracts the tarball, runs `pg_restore --clean --if-exists` into the local PG, and copies RSA keys + attachments to the Vaultwarden data PVC
...
:::caution[Restore CronJob is suspended by default]
The `suspend: true` field prevents the restore from running automatically. This is a safety measure - an accidentally running restore CronJob could overwrite data on a site that has become the primary during failover. Unsuspend it explicitly when setting up the standby, and re-suspend it if the k3s site becomes the primary.
:::
...
### `postgres:alpine` doesn't have `bash`

The `postgres:17.4-alpine` image uses BusyBox, which provides `sh` but not `bash`. CronJob containers that used `bash -c` will fail with `exec: "bash": executable file not found`. Change all `command:` entries to use `sh -c` instead. This applies to both the backup and restore CronJobs.

### Restore CronJob should be suspended by default

The restore CronJob (`suspend: true`) is suspended by default to prevent accidental data overwrites. When the k3s site is being used as a standby, unsuspend it:

```bash
kubectl patch cronjob vaultwarden-restore -n vaultwarden -p '{"spec":{"suspend":false}}'
```

To trigger a one-off restore without unsuspending the schedule:

```bash
kubectl create job --from=cronjob/vaultwarden-restore manual-restore -n vaultwarden
```

### Backup upload filename vs restore download filename

The backup CronJob uploads timestamped tarballs (e.g. `vaultwarden-pg-20260227-050000.tar.gz`) but does **not** upload a `vaultwarden-pg-latest.tar.gz` copy. The restore CronJob downloads `vaultwarden-pg-latest.tar.gz`. This works because the **Servarr primary** backup sidecar uploads both timestamped and `latest` copies. However, if k3s becomes the primary during failover, the k3s backup CronJob only uploads timestamped files - the restore on the other end would fail because there's no `latest` file.

To fix this during failover, either:
1. Manually copy the latest timestamped backup to `vaultwarden-pg-latest.tar.gz` in R2
2. Modify the k3s backup CronJob to also upload a `latest` copy (matching the Servarr sidecar behavior)

### Pin PostgreSQL to a specific Alpine version

Use `postgres:17.4-alpine` (not `postgres:17`) in all k3s manifests. The floating `17` tag pulls the full Debian-based image which is larger and uses `bash` as the default shell. The `-alpine` variant is smaller, uses `sh`, and produces more predictable behavior on ARM64. Pinning the patch version (17.4) prevents unexpected upgrades.
