...
| **Data currency** | n/a - upgraded in place | **as of the backup you pick, not the moment you click** |
...
which shrinks the gap. Neither project used here had PITR enabled, so that half
is documented rather than tested.

Reasoning rather than measurement, but worth stating: it shrinks the gap and
cannot close it. Any instant you pick is still an instant, and the restore is not
instantaneous - the two clones took **100 s and 180 s** from the start of polling
to `ACTIVE_HEALTHY`, and a source under real traffic keeps writing throughout. So
the floor on the gap is the restore duration, not zero.
