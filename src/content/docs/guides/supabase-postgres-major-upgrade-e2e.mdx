...
PG 17 project** (schema dump pre-step, replicate/watch, checksum reconcile,
write-stop cutover). Near-zero downtime and the only cross-region path, but
it lands on a *new* project - new ref, new API keys, new JWT secret, the
same reconfiguration burden as the clone column below. Shown in the diagram
for completeness; the step-by-step here covers the two dashboard tracks.
