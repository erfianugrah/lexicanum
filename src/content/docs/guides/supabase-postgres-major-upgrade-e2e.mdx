...
Setting the list to *empty* is worse than wrong: PostgREST then fails to
build a schema cache at all and serves a steady `503 PGRST002` retry loop
(measured), taking `/graphql/v1` with it, which reads as an outage rather
