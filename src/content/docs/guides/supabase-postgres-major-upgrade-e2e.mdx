...
Re-verifying the table is limited in two ways, established 2026-08-03. The clone
has no Management API endpoint - `database/backups`, `restore`, `restore-pitr`,
...
creation cannot be scripted through it. A project created for the occasion
cannot be cloned either, because the clone needs a completed physical backup and backups
...
| Storage file bytes | untouched | NOT copied - the object row arrives, the bytes do not. Re-confirmed 2026-08-04; the body says `404 NoSuchKey` while the wire status is `400`, so assert on the body |
...
| Data currency | n/a - upgraded in place | as of the backup you pick rather than the moment you click - with PITR, the instant you pick |
...
Measured 2026-08-04, a source with two jobs on `*/5` and `*/15` schedules
produced a clone where both arrived `active = true` and both fired and succeeded
at the first matching tick, six minutes after the clone came up - before anyone
could have opened the dashboard to disable them. The interval between a clone
existing and its jobs running is one cron tick, so disabling them belongs in the
clone procedure itself.

The risk depends on what the job does. Jobs that only touch
...
`cron.job_run_details` is copied like any other table - the clone here inherited 40,843 run records going back four
...
Nothing in the flow warns you how far behind the source the clone is. It is restored from a backup you select, and without PITR the
...
the clone. Under real traffic, every write in that window is missing from the
clone.

With PITR you get a date-and-time selector instead of a list of dailies,[^sb-clone]
and measured 2026-08-04, it closes the gap to the instant you pick. A
...
The restore itself still takes time: that clone took 150 s to reach
`ACTIVE_HEALTHY` (the two non-PITR ones took 100 s and 180 s), and a source under
real traffic keeps writing throughout. The clone does not inherit PITR -
it comes up with ordinary daily backups, so the copy is protected differently
from the thing it was copied from.

Turning PITR on has its own cost. It is refused below Small compute, and the
Micro -> Small resize that gets you there settled in 105 s with 1.0 s of
contiguous REST outage; the Small -> Micro step back settled in 73 s with none
(compute-disk D09, 2026-08-19, 250 ms sampling). The addon
endpoint rate-limits consecutive
changes, so the compute change and the PITR change cannot be applied back to
back.

For a rehearsal, being a few hours behind production changes nothing. Used as a
migration, the clone needs writes stopped at the chosen instant,
...
Settings > API. Symptom if you forget: `406 PGRST106` / "Invalid schema"
errors from the Data API against an otherwise fully restored database.
...
The *Automatically expose new tables* toggle (Integrations > Data API
settings, a.k.a. "Default privileges for new entities") does not help here: it
only auto-grants new tables and functions in an already-exposed schema - it never exposes a new schema. Its mechanism is `ALTER DEFAULT
...
upgrade until dropped). The `pg_cron` jobs copy active, as above; disable
...
This has no user impact. The clone upgrade previews the exact blocker list
and time estimate the production upgrade will show you, so run it even when
the production run looks trivial.
...
7. Green -> schedule the production window. Red -> debug on the clone.
...
initdb/prep). At real database sizes `pg_upgrade
...
GP3 throughput turned into a rate, and the arithmetic gives ~1.3 min
...
The eligibility endpoint does not corroborate the scaling. Read on 2026-08-04,
`duration_estimate_hours` came back as a flat 1 on three small projects, each offering a single patch-level target.
...
None of the reconfiguration list above applies here - the project, URL, keys, JWT secret, auth settings, and storage stay exactly as
...
The upgrade screen does not raise these four platform caveats as
blockers.[^sb-upgrading]
...
DNS, region-specific pooler hostnames) on top of this same pipeline. The
...
The downtime window shrinks to the seconds it takes to drain replication lag,
at the price of the full new-project reconfiguration.
...
(`db.<ref>.supabase.co:5432`); the pooler cannot stream WAL - a subscription
pointed at it fails at the tenant layer with `could not connect to the
publisher ... FATAL: (ENOIDENTIFIER) no tenant identifier provided (external_id
or sni_hostname required)` (edge-resilience W05,
[supabase-lab](https://github.com/erfianugrah/supabase-lab)). Direct hosts are
...
- **Split** (works from a plain IPv4 laptop): `SOURCE_DB_URL` /
  ...
  *direct* host. The subscription connection is dialled by the target's
...
part is required when any replicated table has an FK into
...
The app keeps writing to the source throughout. `watch` aborts on its own if the slot bloats source WAL past the
watchdog threshold - an unconsumed slot retains WAL until the source disk
fills - or the slot's `wal_status` flips to `lost`.

Freeze schema deploys on the source while the subscription runs. In the lab a
source-only `ALTER TABLE` stalled every table's stream - zero rows arrived on
the target, including rows that did not touch the new column - and applying
the same `ALTER` on the target resumed replication in about 6.1 s with the
missed rows backfilled and no subscription recreation (edge-resilience W15,
2026-08-16). If a migration must ship during the window, apply the DDL on the
target first, then on the source.
...
advancing (writes not stopped), waits for lag to reach zero, then
...
Decide the rollback before the write-stop, because it changes at the repoint
(sbshift `docs/RUNBOOK.md`, sections 9 and 12 - a procedure, not a
measurement). Create a named restore point on the source via the Management
API before stopping writes. Up to the moment the app is repointed, rollback is
lossless: `bun start teardown`, re-enable writes on the source, bring the app
back unchanged - the target has taken no application writes. After the
repoint nothing replicates target -> source, so the options are to roll
forward on the target, to repoint back and re-apply the loss window by hand
(never with writes enabled on both), or - if you set it up between
`reconcile` passing and the repoint - to drain reverse replication and cut
back. Reverse replication is a second config with the roles swapped,
distinct slot / publication / subscription names and `copyData: false`, since
the data already matches: `bun start -c migrate.reverse.yaml replicate`. It
is one direction at a time, never active-active, and W05 measured
managed -> managed streaming in one direction only.
...
By hand you lose the gates (readiness audit, wal_level/grant/
...
At larger sizes, a 1,000,000-row initial sync completed
...
(edge-resilience W22, 2026-08-16). Platform-managed schemas (`auth.*`,
`storage.*`) stream zero changes at
any tested size (W09, and the W14 manual drill recorded only in the [edge-resilience RUNLOG](https://github.com/erfianugrah/supabase-lab/blob/713c28f/experiments/edge-resilience/RUNLOG.md)); public and custom tables move fine.
...
3,000 small rows copy in seconds. At real sizes the initial copy takes
longest, and a STORED generated column measured as the throughput
bottleneck (~7x slower copy, ~11 MiB/s vs ~80 MiB/s in sbshift's
...
Module ids are edge-resilience drills and platform-facts probes in
[supabase-lab](https://github.com/erfianugrah/supabase-lab); "RUNBOOK" is
sbshift's `docs/RUNBOOK.md`, a procedure rather than a measurement.

**Before the window**

- Read `duration_estimate_hours` from the upgrade eligibility endpoint and
  publish that as the budget: three aged projects eligible for
  `17.6.1.141 -> 17.6.1.155` each reported `1`, and one project answered
  `eligible: null` with no version fields, so treat null as "not computable"
  rather than false (platform-facts F06, 2026-08-04).
- Do not try to time the in-place window on a project created for the
  purpose: it comes up at the latest version (`eligible: false`,
  `target_upgrade_versions: []`), and `postgres_engine` / `release_channel` on
  `POST /v1/projects` are deprecated and typed `null`, so an older version
  cannot be requested. The clone is the only rehearsal with a real target
  (F06).
- Budget the PITR prerequisite as its own step: PITR is refused below Small
  compute, the Micro -> Small resize settled in 105 s with 1.0 s of contiguous
  REST outage and the step back in 73 s with none (compute-disk D09), and the
  addon endpoint rate-limits consecutive changes, so the compute change and the
  PITR change go in separate windows.
- Create a named restore point on the source via the Management API before
  stopping writes (RUNBOOK section 9).

**While the subscription runs (Path B)**

- Freeze DDL on the source, or apply each migration on the target first: a
  source-only `ALTER` stalled every table's stream, and the same `ALTER` on
  the target resumed it in about 6.1 s with backfill (W15).
- Point the subscription at the direct host only; the pooler refuses with
  `FATAL: (ENOIDENTIFIER) no tenant identifier provided (external_id or
  sni_hostname required)` (W05).
- When scripting the by-hand path through the Management API query endpoint,
  send `CREATE SUBSCRIPTION` as its own single-statement request; the endpoint
  wraps a multi-statement string in one transaction and Postgres refuses
  `CREATE SUBSCRIPTION` inside one (W05).
- If a table sticks in `srsubstate = 'd'` with `pid` null in
  `pg_stat_subscription` (seen only on `auth.*` and `storage.*`, which do not
  replicate managed -> managed; W09, W14), recover in this order: `ALTER SUBSCRIPTION ...
  DISABLE`, `ALTER SUBSCRIPTION ... SET (slot_name = NONE)`,
  `DROP SUBSCRIPTION`, then `pg_drop_replication_slot` on the source -
  `slot_name = NONE` plus drop leaves the publisher slot behind, pinning WAL
  (W09, W14).

**Cutover and rollback**

- Decide the rollback before the repoint: before it, `teardown` plus
  re-enabled source writes is lossless; after it, roll forward, accept the
  loss window, or have reverse replication (`copyData: false`, roles swapped)
  running from between `reconcile` and the repoint (RUNBOOK section 12; W05
  measured one direction only).
- Re-password every custom LOGIN role on the target
  (`ALTER ROLE <name> WITH PASSWORD '...'`) before any service that uses it is
  repointed; the role arrives with `rolpassword` NULL (the region guide's
  measured run; RUNBOOK sections 0 and 9).

**After cutover**

- After `config-sync`, poll the target's config readback until it matches
  instead of reading once - the apply is asynchronous: in the W17 drill a
  readback straight after its restore PATCH returned the pre-restore value
  (edge-resilience battery 2026-08-17) - and diff auth defaults between the two projects:
  `custom_oauth_max_providers` defaulted to 32767 on one project generation
  and 3 on another.
- The paused-window length ("a few days"), `watchdog.maxRetainedWalMb`,
  whether to match compute with `provision`, and when to schedule the window
  are design choices; no run measured them.
...
| Clone storage bytes | GET a pre-existing object via clone ref | 404 (expected) |
...
- **A clone's dashboard lists storage objects whose bytes are missing.** Test a
  GET, not the listing.
...
- **Schema exposure and SMTP are the two forgotten reconfigurations** -
  both fail quietly: nothing flags them at setup, then the API answers `406 PGRST106` on your custom schema, and auth
...
- **pg_cron does not double-fire on the sbshift path** (unlike the clone
  path): `cron.schedule(...)` rows are data, not DDL, so they are
  absent on the target until you re-add them after cutover. The risk is
  forgetting to re-add them.
