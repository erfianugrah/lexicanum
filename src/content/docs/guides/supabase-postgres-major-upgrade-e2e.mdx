...
One schema-restore failure mode worth pre-checking on a PG 17 target: an
operator carrying a non-built-in selectivity estimator cannot be re-created
without superuser, so the restore dies on `must be superuser to specify a
non-built-in restriction estimator function`.[^sb-upgrading] Most projects
have no such operator; the platform doc carries the `pg_operator` query that
...
- **Disk only grows outside an upgrade - and shrinks during one.** You can
  ...
  database on a 200 GB disk comes back on 120 GB.[^sb-upgrading] Note the
  headroom consequence: on gp3 the IOPS you can provision scales with disk
  size (500 extra IOPS per GB), so a right-sized disk lowers that
  ceiling.[^sb-compute-disk]
