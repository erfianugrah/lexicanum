...
compute, and the Micro -> Small resize that gets you there **settled in 105 s
with 1.0 s of contiguous REST outage; the Small -> Micro step back settled in
73 s with none** (compute-disk D09, 2026-08-19, 250 ms sampling). The addon
...
are edge-resilience drills and platform-facts probes in
...
- **Schema exposure and SMTP are the two forgotten reconfigurations** -
  both fail silently-ish: the API answers `406 PGRST106` on your custom schema, and auth
