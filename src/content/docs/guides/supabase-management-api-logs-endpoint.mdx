...
| 6 | **A time window is required, and its absence is the same generic error.** On 2026-09-02 every query without `iso_timestamp_start` and `iso_timestamp_end`, including the example above, answered `Backend error! Retry your query.`; the same SQL with a three-hour window returned rows. Trap 1 makes this indistinguishable from a dialect mistake, so send the window every time. The example in this guide ran without one on 2026-08-22; treat the requirement as current behaviour rather than a documented contract. |
...
| `iso_timestamp_start` / `iso_timestamp_end` are required | measured 2026-09-02 | four queries without them (`edge_logs`, `function_logs`, with and without `LIKE`) -> generic backend error; the same `function_logs` query with a three-hour window -> rows (source `function_logs` carries Edge Function `console.log` lines) |
