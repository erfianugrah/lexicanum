...
| Same query, second run (cached) | 17-24s | <1s |
| Dashboard with 17 simultaneous panels | Timeouts | 3-8s total |

:::note
Structured metadata queries only work on data ingested **after** the Alloy config change. Historical data before the change lacks `downstream_status` and `user_agent` SM fields. The other SM fields (`bot_score`, `request_path`, `router`, etc.) were already being extracted and work on both old and new data.
:::
...
| **Structured metadata** (high-cardinality, 11 fields) | `status`, `downstream_status`, `router`, `service`, `client_ip`, `real_client_ip`, `bot_score`, `request_path`, `duration`, `tls_version`, `user_agent` | `stage.structured_metadata` | Queryable without creating new label streams |
