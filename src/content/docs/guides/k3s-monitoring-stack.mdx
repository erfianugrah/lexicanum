...
| **Structured metadata** (high-cardinality, 9 fields) | `status`, `router`, `service`, `client_ip`, `real_client_ip`, `bot_score`, `request_path`, `duration`, `tls_version` | `stage.structured_metadata` | Queryable without creating new label streams |
