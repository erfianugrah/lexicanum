...
| **Cloudflare Logpush** | **custom gen script** | **125** |
...
```bash
python3 gen-cloudflare-logpush.py   # 125 panels (114 content + 11 section rows)
...
The custom dashboard has 125 panels (114 content + 11 section rows) across 11 sections, generated by `gen-cloudflare-logpush.py`. Every panel has a description tooltip explaining what it shows and how to interpret it.
...
| Bot Analysis | 8 | Bot score distribution, JA4/JA3 fingerprints, verified bot categories, bot score vs WAF action correlation, JS detection results, bot management actions |
| Request & Response Size | 6 | Request/response body size distributions, size by host, size percentiles |
...
:::note
There are ~120k ASNs globally, but each value mapping adds to the dashboard JSON and thus the ConfigMap size (etcd limit: 1.5MB). Including all ASNs would bloat the dashboard beyond the limit. `fetch_asn_names.py` resolves only ASNs actually seen in Loki traffic (~204 entries), keeping the ConfigMap manageable.
:::
...
The cloudflared dashboard has 67 panels (58 content + 9 section rows) across 9 sections, generated by `gen-cloudflared.py`. It covers tunnel health, capacity planning, QUIC transport internals, latency analysis, and process resource monitoring -- all from cloudflared's native Prometheus metrics endpoint.
...
monitoring/
  ...
  grafana/                        # Grafana 12.3.3
    ...
    dashboards/
      ...
      gen-cloudflare-logpush.py   # 125-panel dashboard generator
      gen-cloudflared.py          # 67-panel dashboard generator
