...
category: kubernetes
featured: true
blurb: "Prometheus, Loki, Jaeger, Grafana, and Cloudflare Logpush."
---
...
digraph {
  ...
  subgraph cluster_nodes {
    ...
    subgraph cluster_nodes_rock1 {
      label="rock1"; style=rounded; fontsize=12; margin=12;
      nodes_rock1_alloy1 [label="Alloy\n(logs+traces)"];
      nodes_rock1_ne1    [label="Node Exporter"];
    }
    subgraph cluster_nodes_rock2 {
      label="rock2"; style=rounded; fontsize=12; margin=12;
      nodes_rock2_alloy2 [label="Alloy"];
      nodes_rock2_ne2    [label="Node Exporter"];
    }
    subgraph cluster_nodes_rock3 {
      label="rock3"; style=rounded; fontsize=12; margin=12;
      nodes_rock3_alloy3 [label="Alloy"];
      nodes_rock3_ne3    [label="Node Exporter"];
    }
    subgraph cluster_nodes_rock4 {
      label="rock4"; style=rounded; fontsize=12; margin=12;
      nodes_rock4_alloy4 [label="Alloy"];
      nodes_rock4_ne4    [label="Node Exporter"];
    }
  }

  subgraph cluster_monitoring {
    label="monitoring namespace"; style=rounded; fontsize=12; margin=12;
    monitoring_prometheus   [label="Prometheus v3.9.1\n:9090\nNFS 10Gi\n7d retention"];
    monitoring_alertmanager [label="Alertmanager v0.31.1\n:9093\nNFS 1Gi"];
    monitoring_loki         [label="Loki 3.6.5\n:3100\nNFS 20Gi\n90d retention"];
    monitoring_jaeger       [label="Jaeger 2.15.1\n:4317 OTLP\n:16686 UI\nBadger 10Gi"];
    monitoring_grafana      [label="Grafana 12.3.3\n:3000\n16 dashboards\nAuthentik SSO"];
    monitoring_alloy_lp     [label="Alloy Logpush\n(Deployment)\n:3500", style=dashed];
    monitoring_operator     [label="Prometheus\nOperator v0.89.0"];
    monitoring_ksm          [label="kube-state-metrics"];
    monitoring_blackbox     [label="Blackbox Exporter"];
  }

  subgraph cluster_cf {
    label="Cloudflare Edge"; style=rounded; fontsize=12; margin=12;
    cf_logpush [label="Logpush\n(7 jobs)"];
    cf_tunnel  [label="cfd Tunnel"];
  }

  subgraph cluster_traefik {
    label="Traefik v3.6.8\n(traefik ns)"; style=rounded; fontsize=12; margin=12;
    traefik_decompress [label="Decompress\nPlugin", style=dashed];
  }

  subgraph cluster_vyos {
    label="VyOS Router"; style=rounded; fontsize=12; margin=12;
    vyos_ne_vyos [label="node_exporter\n:9100"];
  }

  nodes_rock1_alloy1 -> monitoring_loki   [label="pod logs"];
  nodes_rock1_alloy1 -> monitoring_jaeger [label="OTLP traces"];
  nodes_rock2_alloy2 -> monitoring_loki;
  nodes_rock3_alloy3 -> monitoring_loki;
  nodes_rock4_alloy4 -> monitoring_loki;

  monitoring_prometheus -> monitoring_alertmanager [label="alerts"];
  monitoring_operator   -> monitoring_prometheus   [label="manages"];
  monitoring_operator   -> monitoring_alertmanager [label="manages"];

  monitoring_prometheus   -> monitoring_grafana;
  monitoring_loki         -> monitoring_grafana;
  monitoring_jaeger       -> monitoring_grafana;
  monitoring_alertmanager -> monitoring_grafana;

  cf_logpush -> cf_tunnel [label="gzip NDJSON"];
  cf_tunnel  -> traefik_decompress [lhead=cluster_traefik];
  traefik_decompress -> monitoring_alloy_lp [label="raw JSON", style=dashed];
  monitoring_alloy_lp -> monitoring_loki;

  monitoring_prometheus -> vyos_ne_vyos [label="ScrapeConfig", style=dashed];
}
```
...
The `tsdb-local` emptyDir volume stores TSDB index and cache on the node's local disk instead of NFS. Index lookups are small random reads, at 1-10ms each over NFS against 0.01-0.1ms on local disk. On pod restart, the index is automatically rebuilt from chunks (takes a few minutes), so ephemeral storage is safe here.
...
:::caution
The TSDB index on emptyDir means Loki takes a few extra minutes to start after a pod restart while it re-syncs the index from the chunk store. This is handled by the `tsdb_shipper.resync_interval` (default 5m). Expect `/ready` to take a few minutes longer than usual after a restart.
:::
...
Structured metadata is a Loki 3.x feature (requires TSDB v13 schema and `allow_structured_metadata: true`). Unlike labels, structured metadata does not affect stream identity - adding a new metadata field does not create new streams or increase index size. That is why IP addresses and request paths go in structured metadata rather than labels.
...
:::note
`disableNameSuffixHash: true` is required: without it, Kustomize appends a content hash to ConfigMap names on every change, creating a new ConfigMap and orphaning the old one. The sidecar would write duplicate files and old dashboards would never be cleaned up.
:::
...
Cloudflare Logpush pushes HTTP request logs, firewall events, and Workers trace events as gzip-compressed NDJSON to an HTTPS endpoint on the cluster, but Alloy's `/loki/api/v1/raw` endpoint does not handle gzip, and Traefik has no built-in request body decompression.
...
The Alloy Logpush receiver runs as a separate Deployment and knows nothing about individual Logpush datasets. Each job injects a `_dataset` field via `output_options.record_prefix`, and Alloy extracts only that as a label:
...
The Performance section's "Edge Processing Time" panel computes `EdgeTimeToFirstByteMs - OriginResponseDurationMs` per log line using Loki's `label_format` with `subf`. Cloudflare caps `EdgeTimeToFirstByteMs` at 65535 (uint16), so when an origin takes longer than ~65 seconds, TTFB saturates while `OriginResponseDurationMs` keeps counting (observed up to 661,156ms). The panel drops the requests hitting this cap (~0.2% of traffic) with `| EdgeTimeToFirstByteMs < 65535`. LogQL pitfall #12 covers both workarounds.
:::
...
The Logpush generator's `http()`, `fw()`, and `wk()` helpers automatically include template variable filter fields plus whatever fields the specific query needs. This reduced query latency on the homelab Loki instance.
...
The fix is the `ScrapeConfig` CRD for direct scraping:
