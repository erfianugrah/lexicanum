...
The cluster runs on 4x ARM64 Rock boards (`rock1`-`rock4`) on a `10.0.0.x` LAN behind a VyOS router with a PPPoE WAN link. All HTTP traffic enters via Cloudflare Tunnel through Traefik. The monitoring stack runs entirely in the `monitoring` namespace.
...
| **Cloudflare Logpush** | **custom gen script** | **91** |
| **Cloudflare Tunnel** | **custom gen script** | **65** |
...
```bash
python3 gen-cloudflare-logpush.py   # 91 panels (82 content + 9 section rows)
python3 gen-cloudflared.py          # 65 panels (56 content + 9 section rows)
```
...
Custom dashboards are generated by Python scripts rather than hand-edited JSON. A 91-panel dashboard is ~5000 lines of JSON but only ~800 lines of Python with reusable helper functions:
...
```python
#!/usr/bin/env python3
"""Generate the Cloudflare Logpush Grafana dashboard ConfigMap YAML."""
import json
from country_codes import COUNTRY_NAMES  # 249 ISO 3166-1 Alpha-2 entries
from asn_names import ASN_NAMES          # 204 entries from Loki traffic
...
def asn_name_overrides(): ...         # ASN number → ISP name
...
The custom dashboard has 91 panels (82 content + 9 section rows) across 10 sections, generated by `gen-cloudflare-logpush.py`. Every panel has a description tooltip explaining what it shows and how to interpret it.
...
| Security & Firewall | 5 | Firewall events by action/source/host, top rules, firewall event timeline |
...
| Threat Intelligence | 5 | Leaked credential pairs, IP classification (Tor/VPN/botnet), geo anomaly on sensitive paths (login/admin/api), client IP reputation |
| Bot Analysis | 6 | Bot score distribution, JA4/JA3 fingerprints, verified bot categories, bot score vs WAF action correlation, JS detection results |
| Workers | 6 | CPU/wall time by script (p50/p95/p99), outcomes (ok/exception/exceeded), subrequest count, execution duration heatmap |
...
**ASN and country name resolution**: Raw ASN numbers and ISO Alpha-2 country codes are mapped to human-readable names using Grafana value mappings. `country_codes.py` has 249 entries (all ISO 3166-1 countries). `asn_names.py` has 204 entries resolved from actual Loki traffic via `fetch_asn_names.py`, which queries Loki for unique ASNs and resolves them through the Cloudflare Radar API (free tier) with ipverse as fallback.
...
The cloudflared dashboard has 65 panels (56 content + 9 section rows) across 9 sections, generated by `gen-cloudflared.py`. It covers tunnel health, capacity planning, QUIC transport internals, latency analysis, and process resource monitoring -- all from cloudflared's native Prometheus metrics endpoint.

| Section | Panels | Key visualizations |
| :------ | :----- | :----------------- |
| Tunnel Overview | 12 stat panels | Requests/sec, error rate %, HA connections, concurrent requests, stream errors/sec, version (`textMode: "name"` to show label), config version, registrations, TCP/UDP sessions, heartbeat retries, total requests |
...
Working with high-cardinality Cloudflare Logpush data in Loki exposed eleven specific traps. These cost real debugging time -- the error messages are often unhelpful.
...
Drop-in replacement for `topk` on instant queries. Results are approximate but accurate enough for dashboard "top N" panels.
...
monitoring/
  ...
  grafana/                        # Grafana 12.3.3
    ...
    dashboards/
      ...
      gen-cloudflare-logpush.py   # 91-panel dashboard generator
      gen-cloudflared.py          # 65-panel dashboard generator
      country_codes.py            # 249 ISO 3166-1 Alpha-2 → country name
      asn_names.py                # 204 ASN → ISP name (from Loki traffic)
      fetch_asn_names.py          # Resolves ASNs via Cloudflare Radar API
...
15. **Python generators >> hand-editing dashboard JSON.** 91 panels = ~5000 lines of JSON, but only ~800 lines of Python with reusable helpers.
...
19. **Selective JSON parsing in LogQL dramatically reduces query cost.** `| json field1, field2` instead of `| json` avoids extracting all ~72 Logpush fields. Each query should only extract the fields it actually uses, plus filter fields for template variables.

20. **`approx_topk` replaces `topk` for high-cardinality Loki queries.** Added in Loki 3.3, uses a count-min sketch for probabilistic aggregation. Requires `query_range.shard_aggregations: approx_topk` (a string, not a YAML list) and `frontend.encoding: protobuf`.

21. **Loki does not watch its ConfigMap.** After config changes, you must `kubectl rollout restart statefulset/loki`. No automatic reload.

22. **cloudflared metrics documentation is unreliable.** `cloudflared_tunnel_active_streams` appears in Cloudflare docs but is not emitted by cloudflared 2026.2.0. Always verify metrics against the actual `/metrics` endpoint before building panels.

23. **cloudflared has no auto-scaling or load balancing.** Replicas are HA only -- Cloudflare's edge does not distribute load across replicas. Scaling down breaks active connections with no graceful drain. For horizontal scaling, use multiple discrete tunnels behind a load balancer.

24. **HTTP tunnels and WARP tunnels scale differently.** HTTP requests multiplex over QUIC streams without consuming host ports. WARP/private network TCP/UDP sessions consume ephemeral ports. Capacity planning must separate these two traffic types.

25. **Prometheus `build_info` metrics need `textMode: "name"`.** The metric value is always `1`; version info is in labels. Stat panels must use `textMode: "name"` to display the version label instead of the number `1`.

26. **ASN name resolution hits ConfigMap size limits.** There are ~120k ASNs globally, but a 1MB etcd ConfigMap limit means you can only include ASN names actually seen in traffic. `fetch_asn_names.py` queries Loki for unique ASNs and resolves only those (~204 entries).
