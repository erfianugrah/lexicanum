---
title: Tuning a USB4 10GbE adapter on Windows
description: A bus-powered USB4/Thunderbolt 10GbE adapter (Alpine Ridge + Marvell AQC113) on Windows - stop RTD3 link flapping, lift the single-flow TX cap with jumbo frames and driver knobs, align L2 and L3 MTU across the link, and assert the whole configuration afterwards so drift shows up as a failure rather than a slow link.
author: "Erfi Anugrah"
category: workstations
featured: true
blurb: "Getting real 10GbE out of a USB4 adapter - the exemplar guide."
---
...
| + CTCP congestion provider | 8.0-8.3 | 9.7 | 9.9 |
...
| Adapter BIOS-side knobs | none: this board exposes no RTD3 disable and no PCIe ASPM option |
...
1. Download the Marvell AQtion driver (3.1.x covers DEV_04C0) from the adapter vendor's download page or the Marvell WHQL pack. The station-drivers Marvell LAN section tracks releases and firmwares.
...
NIC advanced property. EEE on this link produced a 1.3-2.9 Gbps sawtooth; disabling it stabilized the intervals at a steady ~2.7-2.9. It does not raise the cap, it removes noise - and it is a known AQtion instability source, so keep it off regardless.[^archimago-aqc]
...
Vendor corroboration: QNAP's setup docs for their Thunderbolt 3 10GbE adapter (same Thunderbolt-to-AQC recipe) walk through setting MTU 9000 on every OS,[^qnap-qna] and the long-running AQC community tuning guides settle on jumbo + EEE off + RSC off as the stable configuration.[^archimago-aqc]
...
LSO V2, RSC, receive/transmit buffers, flow control, RSS, Windows power plan (Balanced vs High performance), Kernel DMA Protection, Hyper-V root-partition tax (under ~0.4 Gbps), RFC 1323 timestamps. Each was toggled and measured; none moved the number. RSC is worth disabling anyway on AQtion per community reports of episodic disconnects[^archimago-rsc] - harmless here.

:::caution
Never install per-packet shapers (ROG GameFirst, cFosSpeed-class filter drivers) on this box. WFP/filter drivers sit in the TX path and do per-packet processing - exactly the cost this whole guide is minimizing. Check what is bound with `Get-NetAdapterBinding`.
:::
...
Set the router VLAN child to L3 9000, keep the trunk parent at 9014 for tag headroom. On systemd-networkd (NixOS shown, pattern generalizes):
...
Write your end state down as assertions. This script is the one from the rig, generalized; save it as `verify-10g.ps1` and edit the expected remote-address range:
