...
category: networking
featured: true
blurb: "A travel router as a recursive resolver."
---
...
The GL-MT3000 runs OpenWrt (aarch64_cortex-a53) with GL.iNet's firmware layer on top. The OpenWrt `unbound-daemon` package is available in opkg, but the GL.iNet init script manages Unbound via UCI (OpenWrt's Unified Configuration Interface) and generates its own config - it does not use `/etc/unbound/unbound.conf`. This guide works around that by disabling the UCI-managed service and running Unbound directly under a custom procd init script.
...
The fix is to disable the UCI-managed service and run a custom procd init script pointing at `/etc/unbound/unbound.conf`.
:::
