...
group: connectivity
featured: true
blurb: "Private ingress to a serverless container."
---
...
digraph {
  ...
  subgraph cluster_Google {
    label="Google Cloud"; style=rounded; fontsize=12; margin=12;
    Google_glb [label="Google Infrastructure\n(observed: 216.239.x.x)"];

    subgraph cluster_Google_VPC {
      label="VPC: cloudrun (10.0.10.0/24)"; style=rounded; fontsize=12; margin=12;
      subgraph cluster_Google_VPC_Internal {
        label="Internal Services"; style=rounded; fontsize=12; margin=12;
        Google_VPC_Internal_cfd  [label="Cloudflared\n(10.0.10.x)"];
        Google_VPC_Internal_http [label="httpbun\n(10.0.10.y)"];
      }
      subgraph cluster_Google_VPC_Networking {
        label="Network Components"; style=rounded; fontsize=12; margin=12;
        Google_VPC_Networking_router [label="Cloud Router"];
        Google_VPC_Networking_nat    [label="Cloud NAT"];
      }
      subgraph cluster_Google_VPC_Firewall {
        label="Firewall Rules"; style=rounded; fontsize=12; margin=12;
        Google_VPC_Firewall_fr1 [label="TCP:443 -> Google Infrastructure\npriority: 999"];
        Google_VPC_Firewall_fr2 [label="TCP/UDP:7844 -> CF Edge\npriority: 1000"];
        Google_VPC_Firewall_fr3 [label="deny-all\npriority: 10000"];
      }
    }
  }

  edge_ [label="Cloudflare Edge\n198.41.192.0/24\n198.41.200.0/24"];

  Google_VPC_Internal_cfd  -> Google_glb                  [label="(1) Internal Request"];
  Google_glb               -> Google_VPC_Internal_http    [label="(2) Route"];
  Google_VPC_Internal_http -> Google_glb                  [label="(3) Response"];
  Google_glb               -> Google_VPC_Internal_cfd     [label="(4) Return"];
  Google_VPC_Internal_cfd  -> Google_VPC_Networking_router [label="(5) Tunnel"];
  Google_VPC_Networking_router -> Google_VPC_Networking_nat;
  Google_VPC_Networking_nat -> edge_                      [label="(6) Egress"];
}
```
...
# --destination-ranges fetches current Cloudflare IPv4 ranges
...
resource "google_compute_firewall" "allow_cf_traffic" {
  ...
  allow {
    protocol = "tcp"
    ports    = ["7844"]  # TCP (http2) fallback for cloudflared connections
  }
  allow {
    protocol = "udp"
    ports    = ["7844"]  # UDP (quic), the default protocol for cloudflared connections to Cloudflare
  }
