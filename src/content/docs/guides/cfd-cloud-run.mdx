...
flowchart TB
    ...
    subgraph google["Google Cloud"]
        ...
        glb["Google Infrastructure<br>(observed: 216.239.x.x)"]:::loadbalancer
        ...
        subgraph vpc["VPC: cloudrun (10.0.10.0/24)"]
            ...
            subgraph fw["Firewall Rules"]
                direction TB
                fr1["TCP:443 → Google Infrastructure<br>priority: 999"]:::firewall
...
# Note: The exact IP ranges used by Google's infrastructure may vary
# Common observed ranges include 216.239.32.0/19, but you should monitor
# your VPC flow logs to identify the specific ranges in your deployment
...
resource "google_compute_firewall" "allow_google_apis" {
  ...
  # Google infrastructure IP ranges - these may vary by region and service
  # Monitor VPC flow logs to identify specific ranges used in your deployment
  destination_ranges = [
    "216.239.32.0/19",  # Commonly observed range
    "35.190.0.0/16",    # Google APIs and services
    "130.211.0.0/22"    # Additional Google services
  ]
}
