...
| Wire the trust with `oidc_issuer_url` or `jwks_url`, never `custom_jwks`: the third shape returns 201, echoes the key material back and never resolves (`resolved_at` null past 92 s on a fresh pair). Confirm `resolved_at` is populated before routing a tenant to the project. | cross-project-auth X01 (2026-08-03) |
...
| Schedule a hub signing-key rotation as a maintenance window with new sign-ins held. The trusting project's cached kid set did not change across 116 probes in 20 minutes (2026-08-25) or 282 probes in 37 minutes (2026-08-04), and a fresh integration created while the hub's JWKS was current still served the stale set. New sessions break first. Manage the keys at `/v1/projects/{ref}/config/auth/signing-keys`, one standby key at a time (`422 "already has a signing key in standby"`). | key-rotation R02 (2026-08-04 bash; 2026-08-25 live) |
| Pace Management API calls by `x-ratelimit-remaining` (limit 120 per user per scope, 1:1 decrement on a scoped read) and honour `retry-after: 60` on the JSON 429. Do not shard across PATs: the counter is per user, so a second PAT drains the same bucket. | rate-limits L01, L01b (2026-08-17/18) |
| Provision dedicated projects with `region_selection: {type: "smartGroup", code: "apac"}` rather than a pinned city unless a tenant requires one: accepted (201) on a Pro org, landed `ap-northeast-2`, healthy in 135 s. | instance-sizing I02 (2026-08-17) |
| Do not restore a paused free-era project inside a paid org unless it may join the always-on floor: after the wake, `POST /pause` answers `400 Project is not free-tier`. Delete it or move it to a Free org first if it should stay parked. | instance-sizing I03 (2026-08-18) |
...
| Offer JIT database access (`POST /database/jit/invite`) for time-boxed contractor access only on a `platform`-plan org; the identical invite on Pro is rejected with `500`. | sfp-platforms S15 (2026-08-25, A/B) |
