...
description: Reference for running Tailscale alongside IPsec/GRE in a multi-site homelab - declarative NixOS authkey provisioning, Unraid Tailscale SSH, multi-path SSH aliases, access-control idioms, and the failure modes that happened
author: Erfi Anugrah
category: homelab
featured: true
...
**TL;DR:** Tailscale here carries OOB access, per-device identity, and - since the 2026-08-21 cutover - a subnet router that puts every local VLAN one tunnel away from any network. Bulk site-to-site traffic stays on IPsec/GRE. The configurations to copy are the NixOS `authKeyFile` pattern (a rebuilt node rejoins unattended), SSH Host pattern lists (one identity block, three addresses), and the scoped subnet-router firewall rule (tailnet source in, local destinations only, never the WAN). The observed failures were placeholder authkeys, stale tailnet entries in a WSL-generated `/etc/hosts`, an overlay node that was never authenticated in the first place, and routes that are advertised but not forwarded.

<Aside type="note" title="Provenance">
  Every failure mode in this document was observed on the live tailnet and is labelled with its evidence (repo comment, README, or session). External behaviour claims (flag persistence, key expiry, check mode) are cited to the Tailscale documentation, and the nixpkgs module behaviour was checked against the module source. Live state quoted below was re-verified against the running nodes on 2026-08-15; subnet-routing state was re-verified live on 2026-08-23 (router advertising every local VLAN, dev-box client accepting them, `tailscale ping` to two admin-VLAN hosts answering via the router).
</Aside>
...
4. **Reaching devices that cannot run tailscaled.** The router advertises every local VLAN as a subnet route, so the switch, BMCs, and other VLAN-only gear are reachable from any network a tailnet client sits on - enabled 2026-08-21 (see [subnet routing](#subnet-routing)).

Bulk traffic between sites stays on the IPsec/GRE tunnels. They move more data at lower latency because they are routed rather than relayed, and do not depend on fallback via DERP (Designated Encrypted Relay for Packets, Tailscale's relay network) when NAT traversal fails.
...
Until 2026-08-21 every node was box-level: tailnet devices reached tailnet devices, nothing more. The router now also advertises every local VLAN as subnet routes, so tailnet clients reach VLAN-only devices too (the switch, BMCs, anything without tailscaled) - the cutover story is in [subnet routing](#subnet-routing). Its input firewall still treats `tailscale0` as a management interface - SSH, the management UI port, and ping - and its forward chain carries exactly one rule for subnet-routed traffic, scoped to local destinations only.
...
<Aside type="caution" title="Placeholder text in the key file">
...
<Aside type="caution" title="An un-authenticated overlay is not a second path">
  The Pi has an authkey because of an earlier LAN incident (an interface-naming trap on the NIC): the tailnet on that box had never been authenticated, so when the LAN path broke there was no remote path left. The authkey pattern exists so a rebuild can never leave the node in that state.
</Aside>

<Aside type="tip" title="tailscale up flags are not persisted">
...
Authkeys expire after 1-90 days (90 default), so the key on disk is a consumable: if the node ever needs to re-register after the key lapses, generate a new one and drop it in place. An already-registered node stays authorised until its node key expires.[^ts-authkeys] Re-registration also allocates a new tailnet IP: when the Pi was re-registered, its old `100.x` address was gone and anything pinned to it (including a stale `/etc/hosts` entry, below) dialed a dead node.

---

## Subnet routing

Box-level reachability ended 2026-08-21, mid-servarr-cutover: the dev box gave its wired switch port to the new server and moved to the wireless VLAN, which has no route to the admin VLAN by design. The switch, the new server's BMC, every VLAN-only device became unreachable. The router now advertises every local VLAN as a subnet route, and the tailnet is the always-on bypass for any network a client sits on. The config is the router variant above; four steps make the routes carry traffic:
...
During the enablement, each of these signals made a step look done when it was not:
...
`tailscale up` on the router prints a UDP GRO warning for its NIC. It concerns throughput only, and the ethtool fix is optional.
...
The default tailnet policy puts SSH to your own devices in **check mode**: the first connection of the day returns a `login.tailscale.com/a/...` URL and waits for browser re-authentication, then stays authorised for the 12-hour check period.[^ts-ssh] Two operational notes:
...
The SSH config gives each host one block with many addresses. The `Host` line is a pattern list covering every way the host is addressed; the block below it carries the user and identity file. You pick the path by what you type:
...
Three rules keep this scheme working:
...
- **MagicDNS names are patterns, never `HostName`.** Pin `HostName` to the LAN address. The tailnet name stays an alias you can type; the config never depends on it resolving.
