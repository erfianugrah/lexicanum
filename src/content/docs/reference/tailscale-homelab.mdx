...
The idioms that ended up mattering, all from the policy-file syntax reference:[^ts-policy]
...
- **`tests` are free insurance.** A `deny` test on a sensitive subnet fails the policy-file save if a future edit accidentally opens it.
