...
<Aside type="caution" title="Cross-zone is different">
  ...
  entirely. If the origin Worker generates responses directly (e.g., HTML in
  code), there's no `CF-Cache-Status` header since it doesn't go through the CDN
  cache.
</Aside>
...
<Aside type="tip" title="When to centralize">
  Use centralized caching when: - **Origins are third-party APIs** you don't
  control (can't set Cache Rules or `cf` options on their zone) - **Origins
  return `no-store` or `private`** but you know the data is safe to cache -
  **You need to aggregate** responses from multiple origins into a single cached
  response - **Origins are grey-clouded** or non-Cloudflare and you want edge
  caching without changing origin config
</Aside>
