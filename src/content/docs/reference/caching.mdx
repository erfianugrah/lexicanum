...
<Aside type="tip" title="Start with Cache API">
  If per-colo consistency is acceptable, Cache API is significantly simpler than
  KV. Only choose KV if you specifically need global cache coherence or
  metadata-driven purging.
</Aside>
...
<Aside type="caution" title="KV is a commitment">
  Choosing KV means building and maintaining cache infrastructure. Start with
