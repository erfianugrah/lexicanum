...
**Read flow:** Worker looks up the current version, then fetches the manifest for that version, then fetches content by hash.
...
manifest-cache: In-Memory\nmanifest cache

kv: KV Store {
  manifest-v43: "manifest:v43\n/app.js → a1b2c3"
  content-a1b2c3: "content:a1b2c3\n(file bytes)"
}

request -> worker: GET /app.js
worker -> version-cache: 1. get version
version-cache -> worker: v43 or miss
worker -> manifest-cache: 2. lookup path
manifest-cache -> worker: "hash: a1b2c3"
worker -> kv.content-a1b2c3: 3. get content
kv.content-a1b2c3 -> worker: bytes
worker -> request: Response

version-cache -> do: miss {
  style.stroke-dash: 3
}
```

**Write flow:** Publisher writes new content and manifest to KV, waits for propagation, then atomically flips the version pointer.
...
kv: KV Store {
  content-new: "content:d4e5f6\n(new bytes)"
  manifest-v44: "manifest:v44\n/app.js → d4e5f6"
}
...
publisher -> kv.content-new: 1. write content\n(new hash)
publisher -> kv.manifest-v44: 2. write manifest\n(new version)
publisher -> do: 3. wait ~5s\nthen flip to v44
```

The 5-second wait before flipping ensures the new manifest has propagated to most KV edge locations. Without this, some colos might see the new version pointer but fail to find `manifest:v44`.
