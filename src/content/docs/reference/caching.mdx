...
| **Value size limit** | 512 MB | 25 MB (chunking required for larger) |
...
For a production implementation, see [video-resizer](https://github.com/erfianugrah/video-resizer) which implements KV caching with chunking, TTL refresh, cache versioning, and request coalescing. The [Media Transformation Architecture](/reference/media-transformation-architecture/) documents this approach in detail.

For purging KV-cached content by tags or metadata, see [cache-kv-purger](https://github.com/erfianugrah/cache-kv-purger).
