...
ignored -> o2o: Own the domain?
ignored -> cache-api: Per-colo OK?
ignored -> kv: Need global?

o2o: Intermediate domain\n+ O2O {
  rules: Cache Rules apply
  tiered: Tiered Cache works
  no-code: No code changes
}
...
eyeball: Eyeball\neyeball.your-zone.com {
  shape: person
}

worker: Worker/Snippet {
  fetch: "fetch('saas-intermediate\n.your-zone.com')"
}

cache: Calling Zone Cache {
  shape: cylinder
  rules: Cache Rules apply
  tiered: Tiered Cache works
}
...
eyeball -> worker: /saas-path/*
worker -> cache: ownership = same zone
cache -> cname: MISS
cname -> saas: O2O
saas -> cache: response
cache -> worker: cached
...
request -> worker: 1. request
worker -> cache-api: 2. get version
cache-api -> do: miss {style.stroke-dash: 3}
do -> cache-api: v43
cache-api -> worker: v43
worker -> memory: 3. get manifest:v43
memory -> worker: /app.js → a1b2c3
worker -> kv: 4. get content:a1b2c3
kv -> worker: file bytes
worker -> response: 5. respond
```
...
kv: KV {
  shape: cylinder
  content: "content:d4e5f6\n(new file bytes)"
  manifest: "manifest:v44\n/app.js → d4e5f6"
}
...
do: Durable Object {
  shape: hexagon
  version: "version: v43 → v44"
}

publisher -> kv.content: 1. write new content
publisher -> kv.manifest: 2. write new manifest
publisher -> wait: 3
wait -> do: 4. flip version pointer
```
...
cache: Cache API or KV {shape: cylinder}

origins: Origin Workers {
  api: API
  assets: Assets
  auth: Auth
}
...
router -> cache: 2. check cache
cache -> router: 3a. hit → return
router -> origins: 3b. miss → fetch {style.stroke-dash: 3}
origins -> router: 4. origin response
router -> cache: 5. store in cache
router -> client: 6. response
```
...
router -> origins: route to origin
origins -> router: response (cached at origin)
