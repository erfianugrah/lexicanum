...
Choosing KV means building and maintaining cache infrastructure. Start with Cache API unless you specifically need global consistency.
</Aside>
...
title: Content-Addressable KV with DO Version Pointer {
  shape: text
  near: top-center
}
...
read-path: Read Path {
  ...
  worker -> cache-api: 1. get version
  cache-api -> do: miss
  do -> cache-api: "v43"
  worker -> memory: 2. get manifest
  memory -> kv-manifest: miss/stale
  worker -> kv-content: 3. get content
}
...
write-path: Write Path {
  ...
  publisher -> kv-content-w: 1. write content
  publisher -> kv-manifest-w: 2. write manifest
  publisher -> do-w: 3. wait ~5s, flip version
}
```
...
async function getAsset(
  ...
  const { value, metadata } = await env.CACHE_KV.getWithMetadata<{ contentType: string }>(
    `content:${contentHash}`,
    { type: 'arrayBuffer', cacheTtl: 31536000 }
  );
