...
Cache API operations generate separate log entries in the [HTTP requests dataset](https://developers.cloudflare.com/logs/logpush/logpush-job/datasets/zone/http_requests/) with `ClientRequestSource: edgeWorkerCacheAPI` (value 6).

| Cache Operation | Logpush Fields |
|-----------------|----------------|
| `cache.match()` MISS | `EdgeResponseStatus: 504`, `CacheCacheStatus: miss` |
| `cache.match()` HIT | `EdgeResponseStatus: 200`, `CacheCacheStatus: hit` |
...
- **Cache hit rate:** Filter `ClientRequestSource = 6`, then `CacheCacheStatus = hit` vs `miss`
