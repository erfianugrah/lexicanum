...
**[Development Mode](https://developers.cloudflare.com/cache/reference/development-mode/)** temporarily suspends edge caching for up to 3 hours. When enabled, all caching is bypassed - including Cache API operations. Check this first if caching suddenly stops working.

**[Cache Rules](https://developers.cloudflare.com/cache/how-to/cache-rules/)** with "Bypass cache" can override Worker caching behavior. However, Workers can override Cache Rules if the appropriate [compatibility flags](https://developers.cloudflare.com/cache/interaction-cloudflare-products/workers-cache-rules/) are enabled:
...
**[Cloudflare Access](https://developers.cloudflare.com/cloudflare-one/policies/access/)** - Routes protected by Access return `CF-Cache-Status: DYNAMIC` and won't be cached at the edge, regardless of Worker caching logic or `Cache-Control` headers. To enable caching, exclude the path from your Access application policy.
...
Cache API operations generate separate log entries in the [HTTP requests dataset](https://developers.cloudflare.com/logs/logpush/logpush-job/datasets/zone/http_requests/) with [`ClientRequestSource`](https://developers.cloudflare.com/logs/reference/clientrequestsource/) set to `edgeWorkerCacheAPI` (value 6). These entries have `workersubrequest: true` and link back to the parent request via `parentrayid`.
...
The 504 on MISS is [documented behavior](https://developers.cloudflare.com/workers/runtime-apis/cache/#errors-1):
