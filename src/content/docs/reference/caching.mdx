...
<Aside type="tip" title="Start with Cache API">
  If per-colo consistency is acceptable, Cache API is much simpler than
  KV. Only choose KV if you need a single global cache (fewer misses, global
  invalidation), or need to purge by tags/metadata.
</Aside>
...
<Aside type="note" title="R2 vs external origins">
  R2 objects have `.body` (a ReadableStream) that you can `tee()` directly. This
  is more efficient than `fetch()` since R2 is within Cloudflare's network.
</Aside>
