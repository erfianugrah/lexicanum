...
async function fetchWithKV(originUrl: string, env: Env, ctx: ExecutionContext): Promise<Response> {
  ...
  // Use waitUntil for non-blocking write - critical for KV!
  ctx.waitUntil(
...
| KV | Cross-zone, global needed | ~0ms cached | Eventual 60s | [Docs](https://developers.cloudflare.com/kv/concepts/how-kv-works/) |
| DO + KV | Concurrent writes | ~20-50ms | Strong (per-object) | [Docs](https://developers.cloudflare.com/durable-objects/) |
...
1. **Avoid cloning large responses** - If the response exceeds ~50 MB, consider whether caching is appropriate
2. **Use streaming with `tee()`** - `ReadableStream.tee()` creates two streams from one without full buffering
3. **KV has a 25 MB value limit** - Large responses won't fit in KV anyway

```typescript
// For large responses, use tee() instead of clone()
...
While `tee()` avoids immediate full buffering, if one stream is consumed faster than the other, the slower stream's data will be buffered. For very large responses with slow consumers, consider whether caching is the right approach.
</Aside>
