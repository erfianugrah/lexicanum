...
- Dedup handled by queue consumer R2 check (not per-DO flag)
...
11. DLQ consumer marks jobs as `'failed'` when all 10 retries exhausted
...
| 202 Processing                     | Container async (>100 MiB) or reactive 9402       | Return JSON with `jobId`, SSE URL, `Retry-After: 10`  |
...
- **Jobs tab**: auto-discovers container transform jobs from D1, active jobs with SSE progress bars, status filter buttons (All/Active/Complete/Failed with counts), debounced search filter, expandable detail rows with param badges
