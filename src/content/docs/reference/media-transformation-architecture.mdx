...
Container-only features (fps, speed, rotate, crop, bitrate, h265/vp9/av1 codecs, duration >60s) route to the FFmpeg container regardless of file size. All container paths check source size: ≤256 MiB uses sync container (streams through DO); >256 MiB uses async queue (container fetches directly via URL, returns 202 immediately). This prevents DO timeout on large files.
...
digraph {
  ...
  Client     [label="Client"];
  EdgeCache  [label="CF Edge Cache", shape=diamond];
  Middleware [label="Worker Middleware\nvia check -> config -> cdnCgi passthrough -> nonVideo passthrough"];
  Params     [label="Param Resolution\nAkamai translate -> imwidth breakpoint match -> parse (with warnings) -> derivative -> responsive"];
  Origins    [label="Origin Matching\nregex -> capture groups -> priority sources"];
  R2Cache    [label="R2 _transformed/", shape=diamond];
  Coalesce   [label="Per-Isolate Coalescing\nin-flight dedup (bounded FIFO + TTL + 60s timeout, 202 excluded)"];
  Route      [label="Route"];
  Binding    [label="env.MEDIA binding"];
  CdnCgi     [label="cdn-cgi/media"];
  Container  [label="FFmpeg Container DO"];
  Store      [label="R2 put -> R2 get -> cache.put -> cache.match\nzero memory buffering"];

  Client     -> EdgeCache  [label="request"];
  EdgeCache  -> Client     [label="HIT", style=dashed];
  EdgeCache  -> Middleware [label="MISS"];
  Middleware -> Params;
  Params     -> Origins;
  Origins    -> R2Cache;
  R2Cache    -> Client     [label="HIT -> cache.put -> cache.match", style=dashed];
  R2Cache    -> Coalesce   [label="MISS"];
  Coalesce   -> Route;
  Route      -> Binding    [label="R2 <=100 MiB"];
  Route      -> CdnCgi     [label="Remote <=100 MiB"];
  Route      -> Container  [label=">100 MiB or fps/speed/etc"];
  Binding    -> Store      [label="stream"];
  CdnCgi     -> Store      [label="stream"];
  Container  -> Client     [label="202 Processing", style=dashed];
  Container  -> Store      [label="async callback -> R2"];
  Store      -> Client     [label="cache.match (range OK)"];
}
```
...
| R2 persistent store (`_transformed/`) | Global          | Fast, validated via source etag/last-modified     | Via cache.match | Permanent, auto-revalidated |
| KV version registry                   | Global          | Fast                                              | N/A             | Optional manual force-bust |
...
| Header                    | Value                             | Notes                                        |
| ------------------------- | --------------------------------- | -------------------------------------------- |
| `X-R2-Stored`             | `true` or `false`                 | Whether result is durably stored in R2       |
...
| `X-Param-Warnings`        | semicolon-separated               | Validation warnings for rejected params      |
| `Via`                     | `video-resizer`                   | Loop prevention                              |
| `Cache-Tag`               | comma-separated                   | Purge-by-tag support                         |
| `Cache-Control`           | per-status string                 | Per-origin `cacheControl` config overrides auto-generated `public, max-age={ttl}`. Supports `s-maxage`, `stale-while-revalidate`, etc. |

### Mode capabilities

| Feature                   | Video             | Frame                           | Spritesheet | Audio    | Container  |
| ------------------------- | ----------------- | ------------------------------- | ----------- | -------- | ---------- |
| **Dimensions**            | 10-8192px         | 10-8192px                       | Required    | -        | 10-8192px  |
...
Direct streaming from R2 into the Media Transformations binding.[^cf-media-transformations] No HTTP subrequest, no video bytes in Worker memory.
...
For transforms the binding/cdn-cgi can't handle.[^cf-containers] Two sub-modes based on size:
...
- `POST /internal/container-result` -> stores transcoded output in R2, awaits D1 `completeJob` inline (with 1 retry)
...
Container transforms are dispatched via Cloudflare Queue for durability (messages survive deploys, automatic retry with dead letter queue).[^cf-queues]
...
12. DLQ consumer checks R2 first (container may have completed but D1 wasn't updated), marks `'complete'` if found, otherwise `'failed'`
...
| ------------- | ------ | --------------------------------------- | -------------------- |
| `width`       | int    | 10-8192                                 | `?width=1280`        |
| `height`      | int    | 10-8192                                 | `?height=720`        |
...
| ------------------ | ------------------------------ | -------------------------------------------- |
| `imwidth`          | (breakpoint match)             | NOT forwarded as `width` — matched against responsive breakpoints to select a derivative. Any value works, including >8192. |
| `imheight`         | (breakpoint match)             | NOT forwarded as `height` — matched against responsive breakpoints to select a derivative |
...
**Canonical invariant**: derivative dimensions replace any explicit params. `?imwidth=1280` is matched against responsive breakpoints to select a derivative (e.g. tablet), then the derivative's width/height are used for the transform and cache key. `imwidth` is never forwarded as a raw `width` param — values >8192 work correctly because they bypass Zod's 10-8192 validation entirely (breakpoint matching, not direct param forwarding).

### Responsive sizing

When no explicit dimensions are provided, auto-sizing from client signals:

1. **IMQuery breakpoint matching**: `imwidth`/`imheight` values are matched against `config.responsive.breakpoints` (sorted ascending by `maxWidth`). First breakpoint where `imwidth ≤ maxWidth` selects the derivative. Runs before Zod validation so values >8192 work correctly.
2. Client Hints headers (`Sec-CH-Viewport-Width`, `Sec-CH-DPR`, `Width`)
3. `CF-Device-Type` header (mobile/tablet/desktop)
4. `config.responsive.defaultDerivative` fallback
...
```
{mode}:{path}[:w={width}][:h={height}][:t={time}][:d={duration}][:fit={fit}][:a={audio}][:q={quality}][:c={compression}][:fps={fps}][:spd={speed}][:rot={rotate}][:crop={crop}][:br={bitrate}]
```

No version or etag in the cache key — source freshness is validated via R2 metadata (etag/last-modified) on every R2 HIT. Container-only params (fps, speed, rotate, crop, bitrate) are included when present. KV version is appended only on manual force-bust. Derivative name is excluded (only resolved dimensions matter).
...
On every R2 cache HIT, the Worker compares the stored R2 object's etag and last-modified against the current source object. If the source has changed (new upload, different etag), the cached transform is considered stale and a fresh transform is performed. This means source updates automatically invalidate cached transforms without any manual cache busting.

### Cache busting

- **R2 sources**: source etag/last-modified revalidated on every R2 HIT — automatic bust when source changes
...
No `tee()`, no `arrayBuffer()`, no memory buffering. Sequential streaming through R2 then `cache.put`. The final `cache.match` with the original client request (which may include a `Range` header) provides automatic 206 + Content-Range handling for video seeking[^cf-cache-api] — no manual byte math needed.
  ...
  "origins": [
    {
      ...
      "cacheControl": {
        "ok": "public, max-age=86400, s-maxage=604800, stale-while-revalidate=3600",
...
Sources tried in priority order. If one fails (404, 5xx, timeout), falls through to next. Last resort: raw passthrough from any source. Optional `cacheControl` per origin overrides the auto-generated `public, max-age={ttl}` headers, allowing fine-grained control with `s-maxage`, `stale-while-revalidate`, etc.
...
| Binding → container fallback       | Binding MediaError                                | Re-fetch from R2, check size (>256MB → async queue), route to container |
...
| `RequestCoalescer` (signal pattern)   | Per-isolate | Concurrent requests in same isolate. 60s safety timeout prevents stuck entries. 202 async responses excluded (they produce nothing for joiners). |
...
- **Debug Workbench**: form-driven param builder with mode-aware fields, inline media preview (video/image/audio auto-detection), collapsible JSON tree diagnostics, side-by-side comparison mode with header diff, SSE container job progress tracking, Akamai/IMQuery compatibility tester with URL generation, URL hash persistence for shareability
...
```bash
npm run test:run              # 202 unit tests (vitest + workers pool)
...
npm run test:browser          # 49 Playwright browser tests (deploy first)
...
<CardGrid>
  ...
  <Card title="Rocky 640x360 (cdn-cgi)" icon="video">
    ...
    <small>`?width=640&height=360&fit=cover` — via cdn-cgi/media</small>
  </Card>
</CardGrid>
...
<CardGrid>
  ...
  <Card title="Rocky JPEG at 4s" icon="picture">
    <img
      ...
      style="width:100%"
    />
    <small>`?mode=frame&time=4s&width=640&format=jpg` — via binding</small>
  </Card>
</CardGrid>
...
<CardGrid>
  ...
  <Card title="Rocky 640x480" icon="grid">
    <img
      ...
      style="width:100%"
    />
    <small>`?mode=spritesheet&width=640&height=480` — via cdn-cgi/media </small>
  </Card>
</CardGrid>
...
# Cache + range headers
curl -I "https://videos.erfi.io/big_buck_bunny_1080p.mov?imwidth=320"
# -> X-R2-Stored: HIT, cf-cache-status: HIT
...
digraph {
  ...
  Client     [label="Client"];
  EdgeCache  [label="CF Edge Cache", shape=diamond];
  Worker     [label="Worker Middleware + Params"];
  R2Cache    [label="R2 _transformed/", shape=diamond];
  Coalesce   [label="Coalesce + Route"];
  Binding    [label="env.MEDIA binding"];
  CdnCgi     [label="cdn-cgi/media"];
  Container  [label="FFmpeg Container DO"];
  R2Store    [label="R2 put (FixedLengthStream)"];
  R2Async    [label="R2 put (via outbound handler)"];
  CachePut   [label="cache.put"];
  CacheMatch [label="cache.match"];

  Client     -> EdgeCache  [label="request"];
  EdgeCache  -> Client     [label="cache HIT"];
  EdgeCache  -> Worker     [label="cache MISS"];
  Worker     -> R2Cache    [label="check _transformed/"];
  R2Cache    -> Worker     [label="R2 HIT"];
  R2Cache    -> Coalesce   [label="R2 MISS"];
  Worker     -> CachePut   [label="R2 HIT"];
  Coalesce   -> Binding    [label="R2 source <=100 MiB"];
  Coalesce   -> CdnCgi     [label="remote <=100 MiB"];
  Coalesce   -> Container  [label=">100 MiB or fps/speed/etc"];
  Binding    -> R2Store    [label="transform output"];
  CdnCgi     -> R2Store    [label="transform output"];
  Container  -> R2Async    [label="async (202)"];
  R2Store    -> CachePut   [label="stream from R2"];
  R2Async    -> R2Store    [label="callback stores in R2"];
  CachePut   -> CacheMatch [label="cache.put"];
  CacheMatch -> Client     [label="cache.match (range OK)"];
}
```
...
digraph {
  ...
  req1         [label="Request 1 (oversized source)"];
  worker1      [label="Worker"];
  sizecheck    [label="Size > limit?", shape=diamond];
  queue        [label="TRANSFORM_QUEUE"];
  consumer     [label="Queue Consumer"];
  container_do [label="Container DO"];
  container    [label="FFmpeg Container"];
  client1      [label="202 + jobId + SSE URL"];
  download     [label="Download source"];
  ffmpeg       [label="ffmpeg (4 vCPU)"];
  outbound     [label="Outbound handler"];
  r2store      [label="R2 put"];
  d1           [label="D1 (status + percent)"];
  req2         [label="Request 2 (same URL)"];
  worker2      [label="Worker"];
  r2check      [label="R2 check", shape=diamond];
  cacheput     [label="cache.put"];
  cachematch   [label="cache.match"];
  client2      [label="200 video/mp4"];

  req1         -> worker1      [label="imwidth=320"];
  worker1      -> sizecheck;
  sizecheck    -> queue        [label="yes (enqueue)"];
  worker1      -> client1      [label="immediate 202"];
  queue        -> consumer     [label="batch delivery"];
  consumer     -> container_do [label="/transform-url"];
  container_do -> container    [label="start"];
  container    -> download     [label="fetch HTTPS"];
  container    -> d1           [label="progress via /internal/job-progress"];
  download     -> ffmpeg       [label="stream to disk"];
  ffmpeg       -> outbound     [label="callback POST"];
  outbound     -> r2store      [label="FixedLengthStream"];
  outbound     -> d1           [label="complete"];
  req2         -> worker2;
  worker2      -> r2check;
  r2check      -> cacheput     [label="HIT"];
  cacheput     -> cachematch;
  cachematch   -> client2;
}
```
...
digraph {
  ...
  EdgeCache  [label="Edge Cache", shape=diamond];
  Return1    [label="Serve (fastest)"];
  R2         [label="R2 _transformed/", shape=diamond];
  CachePut   [label="cache.put"];
  Transform  [label="Transform (binding/cdn-cgi/container)"];
  R2Store    [label="R2 put"];
  R2Read     [label="R2 get"];
  CacheMatch [label="cache.match"];
  Return2    [label="Serve (with range support)"];

  Request    -> EdgeCache;
  EdgeCache  -> Return1    [label="HIT (cf-cache-status: HIT, X-R2-Stored: HIT)"];
  EdgeCache  -> R2         [label="MISS"];
  R2         -> CachePut   [label="HIT (X-R2-Stored: HIT)"];
  R2         -> Transform  [label="MISS"];
  Transform  -> R2Store    [label="stream (FixedLengthStream)"];
  R2Store    -> R2Read     [label="R2 put complete"];
  R2Read     -> CachePut   [label="stream from R2"];
  CachePut   -> CacheMatch [label="cache.put"];
  CacheMatch -> Return2    [label="cache.match (range OK)"];
}
```
...
digraph {
  ...
  Request     [label="Request"];
  Match       [label="Origin Match?", shape=diamond];
  Sources     [label="Sources by Priority"];
  Source1     [label="Source (remote)", shape=diamond];
  Source2     [label="Source (R2)", shape=diamond];
  Source3     [label="Source (fallback)", shape=diamond];
  Success     [label="Transform"];
  Error404    [label="404 Not Found"];
  Passthrough [label="Raw Passthrough"];

  Request -> Match;
  Match   -> Sources     [label="Yes"];
  Match   -> Error404    [label="No"];
  Sources -> Source1     [label="priority 0"];
  Source1 -> Success     [label="OK"];
  Source1 -> Source2     [label="404/5xx"];
  Source2 -> Success     [label="OK"];
  Source2 -> Source3     [label="404/5xx"];
  Source3 -> Success     [label="OK"];
  Source3 -> Passthrough [label="all failed"];
}
```

[^cf-media-transformations]: Cloudflare, "Transform videos (Media Transformations)," Cloudflare Docs. https://developers.cloudflare.com/stream/transform-videos/
[^cf-containers]: Cloudflare, "Containers," Cloudflare Docs. https://developers.cloudflare.com/containers/
