...
| ------------------ | ------------------------------ | -------------------------------------------- |
| `imwidth`          | (breakpoint match)             | NOT forwarded as `width` — matched against responsive breakpoints to select a derivative. Any value works, including >2000. |
...
**Canonical invariant**: derivative dimensions replace any explicit params. `?imwidth=1280` is matched against responsive breakpoints to select a derivative (e.g. tablet), then the derivative's width/height are used for the transform and cache key. `imwidth` is never forwarded as a raw `width` param — values >2000 (like `imwidth=2160`) work correctly because they bypass Zod's 10-2000 validation entirely.

### Responsive sizing

When no explicit dimensions are provided, auto-sizing from client signals:

1. **IMQuery breakpoint matching**: `imwidth`/`imheight` values are matched against `config.responsive.breakpoints` (sorted ascending by `maxWidth`). First breakpoint where `imwidth ≤ maxWidth` selects the derivative. Runs before Zod validation so values >2000 work correctly.
