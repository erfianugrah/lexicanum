...
- **Blackwell DVFS governor**: the curve sets the frequency ceiling per voltage; the governor picks where the card actually sits - target voltage and actual operating voltage are different things
...
Writes land when GetControl fills the full table (preserving `type_`/`rsvd`), one `freq_offset_kHz` is modified, and SetControl writes the whole table back. Then verify by read-back - a write that doesn't read back is an error, not a success.
...
The telemetry answered the question the curve data couldn't: the card sits at 930 mV with the curve targeting 860 because Blackwell's DVFS governor adds voltage margin under sustained load, independent of the curve. Power (103W max of 575W) and temperature (61C) ruled out both limiters. The voltage step happens the instant utilization crosses ~85%, before power or clock change - it follows load duration, not demand.
