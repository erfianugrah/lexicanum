...
2026-09-08 on the same engine and artifact. Every claim below is measured unless
...
Two accuracy questions are still open. Long-context *quality* (as opposed to
the proven flat decode) needs a needle-in-context probe, which is built but
blocked: NInfer serves no `/tokenize` endpoint, so the probe's filler sizing
had to grow a local HF tokenizer fallback, and the probe's per-context
ephemeral-swap design assumes a hot-swappable engine NInfer is not - it needs
a no-swap mode that probes the resident model at its current context. And
churn stability beyond a single session is unmeasured: 6.2% of requests in the
validation run decoded below 100 tok/s (worst 33.9), clustered at 66-72K
context, and the cause is unexplained.
