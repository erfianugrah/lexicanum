...
description: Measured answers for running Qwen3.8-27B under an agentic loop harness - why multi-token prediction (MTP) speculative decoding degrades within minutes of agentic churn, why the draftless ngram-mod fails the same way, what reasoning_effort=medium actually costs and buys, and which popular config advice from the release-week threads fails on contact with measurement
author: Erfi Anugrah
...
- **Quantising the KV cache to stretch context** (q4_1 main, 73k context on 16 GB). A 16 GB workaround; at 32 GB the full 196k x 2 slots fit without it, and q4 main KV costs long-context recall.
