...
Number of variants, number of requests, cache hits or misses, egress: none of them move this metric. The only lever on the origin-image count is how many distinct files get rendered in the cycle. The docs' own "Optimize usage" section (pre-generate variants, CDN caching, Cache-Control) reduces compute and egress but does nothing for this line item.[^sb-imgt-billing]
...
- **Rate limiting exists but the floor is high.** 200 parallel fresh renders all returned 200; at 500 parallel, 11 came back 429 (~2%); at 1,000 parallel, 88 (~9%). Refusals ramp rather than cut off, so a determined burst still lands most of its requests. The same ceiling bounds your own bulk work: keep pre-warming and batch renders at or under 200 concurrent requests and retry on 429. It was measured on one project and one source, so whether the ceiling is per project, per IP or per org is open.
...
Two properties of a migration off transforms matter for planning. First, **usage tapers rather than stops**: if your traffic comes from installed clients (mobile apps, third-party embeds), old versions keep calling transform URLs until they are upgraded, and every distinct image they touch still counts - budget the remainder of the cycle, not a clean stop. Second, **the meter stays open after your own usage ends**: moving to upload-time renditions zeroes your clients' contribution, but on a public bucket anyone else can still hit the render endpoint and re-create billable origin images. The post-migration signal on the usage page is binary - your clients contribute nothing, so any origin-image count above zero is someone else.

Two workarounds that do **not** work, despite appearing in the docs' optimisation advice:[^sb-imgt-billing] pre-generating variants *while still serving transform URLs* (the origin still counts the first time anything renders it - the rendition pipeline only works because it drops the render endpoint from the read path entirely) and CDN caching (per-origin counting is request-independent).

## What to do about it

The mitigations table ranks the billing posture. These are the operating practices on top of it, each with the measurement it rests on.

| Practice | Rests on |
|---|---|
| Keep pre-warming and batch renders at or under 200 concurrent requests, and retry on 429 | I09 (500 and 1,000) and the ad-hoc probe (200), Evidence row "Rate ceiling": 0 of 200, 11 of 500 and 88 of 1,000 parallel fresh renders answered 429. Measured on one project and one source; whether the ceiling is per project, per IP or per org is open |
| Reject sources over 25MB or 50 megapixels at upload time, in the client or an upload-path function | I04 and the ad-hoc probe, Evidence row "25MB and 50MP source limits enforced": the ad-hoc 28.8MB PNG and 56MP JPEG, and I04's generated >25MB and >50MP PNGs, uploaded without complaint and returned 400 only at render |
| Add a test that fetches every transform URL your app builds and asserts the path contains `/render/image/` and the body is smaller than the original | I02, Evidence row "`/object/public` and `/object/sign` ignore appended params": full original bytes with a 200 and no error |
| Give each output format its own variant URL; do not let webp and jpeg clients share one render URL | I05, Evidence row "No `Vary: Accept`": webp and png served from one URL, `vary` header absent. Whether the `format` transform option pins the edge entry was not measured |
| Enforce your own maximum variant size in the URL builder; the server clamps and never errors | I03 and the ad-hoc probe, Evidence row "Dimension clamp at 3000 / source dims": widths 2500, 2501, 3000, 3001 and 5000 on a 4000x3000 source, none rejected |
| Version the object path (`img-v2.jpg`) when an image changes; do not rely on overwrite invalidation | I06, Evidence row "Overwrite invalidation unreliable": 4 of 5 trials served the stale variant past the poll window (three 60s polls, one 15s poll) |
| Send `x-upsert: true` (supabase-js `upsert: true`) when overwriting in place, and check the 200 before reading any effect | I06 harness note: a Storage POST to an existing path without it returns 400, and a failed overwrite followed by a warm hit reads as a stale cache |
| Apply the private-bucket posture on Free-plan projects too; plan gating does not protect the render surface | Evidence row "Free-plan org renders": 200 with resized output on a Free-plan scratch project (ad-hoc probe, not yet a scripted module) |
| Gate `/render/image/authenticated` with a bucket-scoped `storage.objects` select policy, and test the denial as well as the allow | I08, Evidence row "Authenticated render enforces storage RLS": a user JWT was denied without a select policy and rendered with one |
