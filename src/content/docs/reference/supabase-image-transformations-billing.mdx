---
title: "Supabase Storage image transformations: billing, runtime behaviour, and the abuse surface"
description: How Supabase bills Storage image transformations - per distinct origin image per billing cycle, in 1,000-image packages past a 100-image quota - measured end to end on scratch Pro and Free orgs. Which URL surfaces transform, where the documented limits disagree with the runtime, how the edge cache behaves, and the denial-of-wallet surface on public buckets with ranked mitigations.
author: Erfi Anugrah
category: supabase
group: operations
---
...
Supabase bills Storage image transformations per distinct origin image per billing cycle, rather than per transformation or per request.[^sb-imgt-billing] Re-rendering is therefore free, the 101st image in a cycle costs \$5, and on a public bucket the count is influenceable by anyone who can read your HTML. The measured runtime behaviour disagrees with the docs in three places.
...
The billing docs define the model in three rules.[^sb-imgt-billing]
...
The docs say width and height must be integers between 1 and 2500.[^sb-imgt] At runtime, `width=2501` on a 4000x3000 source returns a 2501px render. The actual ceiling is 3000px: requests above that silently clamp to 3000, and requests above the source dimensions silently clamp to the source (no upscaling, no error). The documented bound is too low as a limit, and the runtime never errors on it.

The docs also say "Image Resizing is currently enabled for Pro Plan and above."[^sb-imgt] The scratch Free-plan org rendered a 200x200 transform with a 200 on the first request. Either the doc is stale or free-plan rendering is gated by Fair Use rather than a hard entitlement - but nothing at the render surface checks the plan. The billing page's quota table also omits the Free plan entirely.[^sb-imgt-billing]
...
The spend cap decides the outcome:[^sb-cost-control]
...
| Your own edge in front (Cloudflare proxy/WAF/rate rules) | rate and pattern control on the render path | origin images rendered through it still count; it controls requests and leaves the count alone |
...
Two workarounds fail despite appearing in the docs' optimisation advice:[^sb-imgt-billing] pre-generating variants *while still serving transform URLs* (the origin still counts the first time anything renders it - the rendition pipeline only works because it drops the render endpoint from the read path entirely) and CDN caching (per-origin counting is request-independent).
