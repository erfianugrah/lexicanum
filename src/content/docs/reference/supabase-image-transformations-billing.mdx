...
Two workarounds that do **not** work, despite appearing in the docs' optimization advice:[^sb-imgt-billing] pre-generating variants (the origin still counts the first time anything renders it, and serving stored variants only helps if you stop using the render endpoint entirely) and CDN caching (per-origin counting is request-independent).
