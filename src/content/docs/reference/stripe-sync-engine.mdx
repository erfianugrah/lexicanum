...
The Supabase Model Context Protocol (MCP) server exposes this data to an LLM through `execute_sql`, scoped by URL parameters `project_ref`, `read_only` and `features`.[^sb-mcp] Two findings matter if you are building on that.
