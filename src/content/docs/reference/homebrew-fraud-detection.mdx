...
Train: RF 20 trees {shape: hexagon}
Cal: Platt Cal
...
## Abuse-operations maturity: verified gaps

The sections above describe a strong **per-submission scorer**. Measured against the
broader *abuse-operations* doctrine (Heroku's "Hunting Wolves in Sheep's Clothing" +
the Facets/First-Principles model), the system's gaps are not in detection mechanics
but in **actor- and campaign-level operations**. Every gap below was verified against
production code at the cited path.

<Aside type="note" title="Root cause">
The pipeline is **per-event, not per-actor**. Each submission is scored in isolation;
the strongest cross-submission memory is the blacklist. The doctrine's unit of work is
the **campaign footprint** tracked across many submissions. Most gaps below follow from
this.
</Aside>
...
| --- | --- | --- |
| **No campaign / actor footprint layer** — no pivot / splash / retool detection | Confirmed | Schema has 5 tables (`submissions`, `turnstile_validations`, `fraud_blacklist`, `fraud_blocks`, `fingerprint_baselines`) — `migrations/0001_initial_schema.sql`. No campaign/actor table. A reference `matchFootprint` (N-of-M) matcher exists at `src/lib/footprint.ts` but is imported **only** by `tests/footprint.spec.ts` — not wired into the pipeline. |
| **No intent taxonomy** — abuse collapses to a 0–100 magnitude | Confirmed | `monetisation\|weaponi\|disruption\|reputation\|victimi\|intentAspect` → **0 matches** in `src/`. No classification of *what kind* of abuse. |
| **Binary block, no analyst review band** | Confirmed | Decision is `riskScore >= config.risk.blockThreshold` (`src/lib/config.ts:469`). `levels: {low/medium/high}` exist but are **severity display labels**, not an action band. No review queue. |
| **Banned actors return as fresh unknowns** (First Principle 5) | Confirmed — worse than expected | `cleanupExpiredBlacklist` runs `DELETE FROM fraud_blacklist WHERE expires_at <= ?` (`src/lib/fraud-prevalidation.ts:363`). A **hard delete** — the row *and its `offense_count`* vanish on TTL. Consequence: the progressive-timeout escalation (1h→4h→…→24h) can never progress for an actor who simply waits out each window. |
| **No retroactive sweep** (non-repetition) | Confirmed | `sweep\|retroact\|backfill` → 0 matches. All scheduled `DELETE FROM` ops are cleanup, never re-detection over history. |
| **Outbound = inbound not addressed** (First Principle 7) | Confirmed | Blocking only; no downstream-notify or loot-devaluation path. A flagged credential-harvest payload is blocked but not reported/neutralised. |
| **Blacklist is self-generated only** | Confirmed | Populated solely via `addToBlacklist` on a block (`src/routes/submissions.ts`). No external-feed ingestion ("seen abusive elsewhere first" — the sanctioned pre-emptive signal under First Principle 2). |

### What it already does well (for contrast)

Reversibility (First Principle 8) is **satisfied** — progressive timeouts are inherently
reversible. Codifiability (First Principle 9) is **mostly satisfied** — every decision
stores a full `ScoringDecision` audit trail and rules are config/KV-driven with no redeploy.

### Improvement roadmap (sized against the codebase)

| # | Change | Effort | Touches | Risk |
| - | ------ | ------ | ------- | ---- |
| 2 | **Retain-after-expiry tier** (First Principle 5): soft-expire instead of hard-delete; resume `offense_count` on return | S — ½–1 day | migration `0002` (+`status`), `fraud-prevalidation.ts` (~25 LOC) | Low |
| 3 | **Intent-aspect tagging**: enum + derive aspects from fired components, persist in audit trail | S–M — ~1 day | `types.ts`, `scoring.ts` (~40 LOC), migration, `submissions.ts` | Low |
| 4 | **Analyst review band** (decision only): `block \| review \| allow` via a `reviewThreshold` | S — ½ day | `config.ts` | Low |
| 1 | **Footprint layer (MVP)**: signal→`Indicator` mapper, load/match/store against seeded footprints | M — 1.5–2 days | migration (`fraud_footprints`, `footprint_matches`), new `footprint-integration.ts` (~120 LOC), `submissions.ts` Phase 2 (~40 LOC), `database.ts` | Med (hot path) |
| 5 | **Retroactive sweep**: re-run a new footprint over recent submissions | M — 1–2 days | scheduled task. **Prereq:** persist full raw signals — today only `risk_score` + breakdown are stored | Med |
| 6 | **External-feed ingestion** (disposable-email / known-bad IP-JA4) with a `source` tag | M — 1–2 days | scheduled fetch → blacklist | Low |
| 4+ | Review band **full workflow** (queue + dashboard view + approve/reject endpoints) | L — 3–4 days | React + Hono | Med |
| 6+ | **Demotivation / loot-devaluation** (downstream notify, ROI destruction) | L+ — design-heavy | downstream-specific | High |

<Aside type="tip" title="Cheapest high-value slice">
Items **2 + 3 + 4-decision** together (~2–2.5 days, one migration, ~70 LOC, no UI, no new
infra) ship three First-Principles fixes — return-tracking, intent classification, and a
review band — touching nothing on the hot path. The **swing factor is UI**: every
"log it / score it" change is small; every "analyst works a queue" change is 3–4× larger.
A headless roadmap (log to D1, read via dashboard) is ~1 week; a full analyst workflow is
2–3 weeks.
</Aside>
