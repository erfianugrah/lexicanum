...
<Aside type="caution" title="Write-before-read">
  ...
  collecting signals. Otherwise you have a TOCTOU race where the first request
...
Bigram and trigram frequency analysis across 8 language groups (English, Spanish, French, German, Italian, Portuguese, Romanized, and a universal/mixed set). Input is NFD-normalized to strip diacritics while preserving base characters, reducing false positives on international names. Scores how likely a character sequence is to appear in natural text for each language, taking the best match. Language detection uses suffix-based heuristics (e.g., `-ez` for Spanish, `-stein` for German, `-ovich` for Russian). Higher naturalness = lower fraud risk.
...
The model (20 trees, trained both offline via scikit-learn and online via a container-based TypeScript CART implementation) is serialized as JSON and stored in KV. The Worker evaluates it using iterative tree traversal (not recursive, to avoid stack limits on the edge runtime).
...
Calibration parameters (`intercept`, `coef`) are stored in the model's metadata and applied automatically during inference. The production model's calibration was fitted on 50,000 OOB samples.
