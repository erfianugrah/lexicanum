---
title: "We have fraud detection at home"
description: Architecture reference for building a multi-layer form fraud detection system with 48-feature Random Forest email classification, TLS fingerprinting, container-based model retraining, and weighted risk scoring on Cloudflare Workers
author: Erfi Anugrah
category: cloudflare
group: edge
featured: true
blurb: "Random Forest email classification, TLS fingerprinting, and weighted risk scoring on Workers."
---

import { Aside } from "@astrojs/starlight/components";

How [**forminator**](https://github.com/erfianugrah/forminator) and [**markov-mail**](https://github.com/erfianugrah/markov-mail) implement multi-layer fraud detection for form submissions on Cloudflare Workers. Two Workers - one handling form submissions with multi-layer detection and 10-component risk scoring, the other running a 48-feature Random Forest classifier on email addresses with automated weekly retraining - connected via RPC service bindings.
...
digraph {
  ...
  Bot     [label="Bot Mgmt", shape=ellipse];
  BL      [label="Blacklist", shape=diamond];
  TR      [label="Token Replay", shape=diamond];
  TV      [label="Turnstile"];
  TS      [label="Turnstile API"];
  Signals [label="Collect Signals"];
  ML      [label="ML Worker", shape=hexagon];
  Score   [label="Score (10w)", shape=hexagon];
  D1      [label="D1", shape=cylinder];

  Browser -> BL;
  Bot     -> TV      [style=dashed];
  BL      -> TR      [label="clean"];
  TR      -> TV      [label="clean"];
  TV      -> TS      [label="validate"];
  TS      -> TV      [label="eph_id"];
  TV      -> Signals;
  Signals -> ML      [label="RPC", style=dashed];
  ML      -> Score   [label="score", style=dashed];
  Signals -> Score;
  Score   -> D1;
}
```
...
| **ML Worker**       | Email fraud classification, feature extraction, model inference | Hono on Workers, D1, 3x KV, Containers       |
...
<Aside type="caution" title="Enterprise features">
  Ephemeral IDs from Turnstile and JA4/bot score signals from Bot Management
  require Enterprise plans. The system works without them - those layers
  degrade gracefully and their scoring weight is redistributed to active
  signals.
</Aside>
...
digraph {
  ...
  BL      [label="Blacklist", shape=diamond];
  TR      [label="Token Replay", shape=diamond];
  TS      [label="Turnstile", shape=diamond];
  Write   [label="Write Record"];
  Dup     [label="Duplicate?", shape=diamond];
  Signals [label="Collect Signals"];
  Email   [label="Email (RPC)"];
  Eph     [label="Ephemeral ID"];
  JA4     [label="JA4 Fingerprint"];
  IPFP    [label="IP + Fingerprint"];
  Score   [label="Weighted Score", shape=hexagon];
  Decide  [label="Threshold?", shape=diamond];
  Allow   [label="201"];
  Block   [label="429 + Blacklist"];

  BL      -> TR      [label="clean"];
  TR      -> TS      [label="clean"];
  TS      -> Write   [label="pass"];
  Write   -> Dup;
  Dup     -> Signals [label="unique"];
  Dup     -> Block   [label="3x+", style=dashed];
  BL      -> Block   [label="hit", style=dashed];
  ...
  IPFP    -> Score;
  Score   -> Decide;
  Decide  -> Allow   [label="below"];
  Decide  -> Block   [label="above"];
}
```
...
<Aside type="tip" title="Why a fast path matters">
  Turnstile validation costs an external API call. If you've already determined
  an identifier is fraudulent, checking a local D1 table first avoids that cost
  entirely. In practice, this can eliminate the API call for the majority of
  repeat offenders.
</Aside>
...
<Aside type="note" title="Fail-open design">
  Email fraud detection fails open - if the ML Worker is unavailable, the
  submission proceeds with a 0 risk score for that component. Its weight is
  redistributed to the remaining active signals. Token replay and Turnstile
  validation are the only fail-secure checks.
</Aside>
...
<Aside type="caution" title="Write-before-read">
  The current request's validation record must be inserted into D1 **before**
  collecting signals. Otherwise you have a time-of-check/time-of-use (TOCTOU) race where the first request
  from a device always sees count=0. Insert the pending record first, then
  query.
</Aside>
...
<Aside type="tip" title="IPv6 subnet matching">
  For IP clustering, compare IPv6 addresses at the /64 prefix level (first 4
  groups). Privacy extensions (RFC 8981) rotate the interface identifier but
  keep the prefix stable, so /64 matching links requests from the same network
  without relying on a stable full address.
</Aside>

<Aside type="note" title="Clustering includes blocked requests">
  JA4 clustering queries both `submissions` and `turnstile_validations` tables
  via `UNION ALL`. This ensures attackers who are repeatedly blocked by the
  blacklist fast path (and therefore never create submissions) still escalate
  the clustering score for their JA4 fingerprint.
</Aside>
...
<Aside type="note" title="IP rate limit is never a standalone block trigger">
  The IP Rate Limit component contributes to the weighted risk score but never
  independently triggers a block. This prevents false positives from shared IPs
  (offices, universities, NAT). Blocks only occur when the combined risk from
  all signals exceeds the threshold.
</Aside>
...
digraph {
  ...
  Raw    [label="10 Components"];
  Redist [label="Redistribute", shape=diamond];
  Corr   [label="Corroboration", shape=diamond];
  Det    [label="Deterministic", shape=diamond];
  Final  [label="Final 0-100", shape=hexagon];
  Decide [label="Threshold?", shape=diamond];
  Allow  [label="Allow"];
  Block  [label="Block"];

  Raw    -> Redist [label="base"];
  Redist -> Corr   [label="normalized"];
  Corr   -> Det    [label="adjusted"];
  Det    -> Final;
  Final  -> Decide;
  Decide -> Allow  [label="below"];
  Decide -> Block  [label="above"];
...
| `ephemeral_id_fraud`   | Requires elevated ephemeral ID score AND validation frequency      |
...
<Aside type="tip" title="Sanitize before fraud detection">
  Run sanitization before the fraud pipeline so detection signals operate on
  clean input. The Turnstile token is explicitly excluded from sanitization.
</Aside>
...
The ML Worker extracts a 48-dimension feature vector from an email address and scores it with a Random Forest classifier. It serves as both a standalone HTTP API and an RPC service.

### Feature vector

Features span ten categories:

| Category            | Examples                                                                   | Count |
| ------------------- | -------------------------------------------------------------------------- | ----- |
| **Sequential**      | `sequential_confidence`, `plus_risk`                                       | 2     |
| **Local Part**      | `local_length`, `digit_ratio`, `has_year_suffix`, `alpha_prefix_naturalness`, `digit_position_ratio` | 5     |
| **Identity**        | `name_similarity_score`, `name_token_overlap`, `name_in_email`             | 3     |
| **Geo**             | `geo_language_mismatch`, `geo_timezone_mismatch`, `geo_anomaly_score`      | 3     |
| **MX/Provider**     | `mx_has_records`, `mx_record_count`, `mx_provider_*` (8 one-hot), `provider_is_free`, `provider_is_disposable` | 12    |
| **Domain/TLD**      | `tld_risk_score`, `domain_reputation_score`, `entropy_score`               | 3     |
| **Linguistic**      | `pronounceability`, `vowel_ratio`, `max_consonant_cluster`, `repeated_char_ratio`, `syllable_estimate`, `impossible_cluster_count` | 6     |
| **Structure**       | `has_word_boundaries`, `segment_count`, `avg_segment_length`, `segments_without_vowels_ratio` | 4     |
| **Statistical**     | `unique_char_ratio`, `vowel_gap_ratio`, `max_digit_run`, `bigram_entropy`  | 4     |
| **N-gram**          | `ngram_bigram_score`, `ngram_trigram_score`, `ngram_overall_score`, `ngram_confidence`, `ngram_risk_score`, `ngram_is_natural` | 6     |
...
digraph {
  ...
  Req      [label="POST /validate"];
  HasEmail [label="email?", shape=diamond];
  Skip     [label="pass through"];
  Load     [label="Load from KV"];
  Feat     [label="48 Features"];
  RF       [label="Random Forest", shape=hexagon];
  Cal      [label="Platt Cal"];
  OOD      [label="OOD?", shape=diamond];
  Heur     [label="Heuristics"];
  Decide   [label="Decision", shape=diamond];
  D1       [label="D1", shape=cylinder];
  Next     [label="Route Handler"];

  Req      -> HasEmail;
  HasEmail -> Load [label="yes"];
  HasEmail -> Skip [label="no"];
  Load     -> Feat;
  Feat     -> RF;
  RF       -> Cal;
  Cal      -> OOD;
...
The 48 features are produced by specialized detector modules. Each operates independently and returns structured results.
...
Bigram and trigram frequency analysis across 8 language groups (English, Spanish, French, German, Italian, Portuguese, Romanized, and a universal/mixed set). Input is NFD-normalized (Unicode Normalization Form D) to strip diacritics while preserving base characters, reducing false positives on international names. Scores how likely a character sequence is to appear in natural text for each language, taking the best match. Language detection uses suffix-based heuristics (e.g., `-ez` for Spanish, `-stein` for German, `-ovich` for Russian). Higher naturalness = lower fraud risk.
...
- **Resolution**: Cloudflare DNS-over-HTTPS (`cloudflare-dns.com/dns-query`) with 500ms fetch timeout via `AbortController` (wrapped in a 1500ms middleware timeout with background retry)
...
- **Features produced**: `mx_has_records`, `mx_record_count`, and one-hot encoded `mx_provider_*` flags (8 providers: Google, Microsoft, iCloud, Yahoo, Zoho, Proton, self-hosted, other). Cache bounded to 10k entries with LRU eviction.

<Aside type="note" title="Well-known provider fast path">
  Common domains (gmail.com, outlook.com, yahoo.com, etc.) skip DNS entirely and
  return cached provider classifications. This avoids the DoH call for the vast
  majority of legitimate email addresses.
</Aside>
...
```json
{
  "riskThresholds": { "block": 0.88, "warn": 0.56 },
  "actionOverride": null,
  "adjustments": {
...
The model (20 decision trees of the classification-and-regression (CART) kind, trained both offline via scikit-learn and online via a container-based TypeScript implementation) is serialized as JSON and stored in KV. The Worker evaluates it using iterative tree traversal (not recursive, to avoid stack limits on the edge runtime).
...
<Aside type="tip" title="Hot-swappable models">
  Storing the model in KV means you can deploy a new model without redeploying
  the Worker. The config loader caches the model with a TTL (default 60 seconds
  per KV's caching behavior). Upload a new model JSON to KV and it propagates
  globally within the eventual consistency window.
</Aside>
...
Calibration parameters (`intercept`, `coef`) are stored in the model's metadata and applied automatically during inference. The production model's calibration was fitted on 50,000 out-of-bag (OOB) samples.
...
The two Workers communicate via Cloudflare's service bindings,[^cf-service-bindings] which use Workers RPC[^cf-workers-rpc] built on Cap'n Proto. Per the docs, there is zero overhead - both Workers execute on the same thread of the same server.
...
<Aside type="note" title="Single code path">
  The RPC entrypoint constructs a synthetic HTTP `Request` and feeds it through
  the same Hono app. This means HTTP API users and RPC callers execute identical
  validation logic - no divergent code paths to maintain.
</Aside>
...
digraph {
  ...
  Block  [label="Blocked"];
  Write  [label="Write Entry"];
  Active [label="Active?", shape=diamond];
  L0     [label="Layer 0", shape=diamond];
  Update [label="Escalate"];
  Expire [label="Expired"];
  Free   [label="Cleared"];

  Block  -> Write;
  Write  -> Active;
  Active -> L0     [label="next hit"];
  L0     -> Update [label="high/med"];
  L0     -> Active [label="low", style=dashed];
  Update -> Active;
  Active -> Expire [label="TTL"];
  Expire -> Free;
}
```
...
<Aside type="caution" title="Secrets vs vars">
  API keys and CAPTCHA secrets go in `wrangler secret put`, not in the config
  file. The `FRAUD_CONFIG` var is safe in wrangler.jsonc since it only contains
...
| `fraud_blacklist`       | Progressive mitigation cache      | email, ephemeral_id, ja4, ip_address, expires_at, detection_confidence |
...
| Table              | Purpose                                                                                        |
| ------------------ | ---------------------------------------------------------------------------------------------- |
| `validations`      | Every email validation with all signals and features (86 columns)                              |
| `training_samples` | Auto-collected 48-feature vectors from live validations, with labels and source tracking        |
| `training_metrics` | Model training history: accuracy, precision, recall, F1, FPR, anomaly detection                |
| `ab_test_metrics`  | Experiment tracking for model variants                                                         |
| `admin_metrics`    | Configuration change audit log                                                                 |

The `training_samples` table drives the automated retraining loop. Each validation writes a feature vector with an auto-assigned label (`known_disposable`, `known_provider`, or `model`) keyed by a unique email hash. Manual review labels get 5x weight during training. Samples older than 14 days are pruned after each retraining cycle.

<Aside type="note" title="D1 constraints">
  D1 databases are single-threaded and backed by a single Durable Object with a
  10 GB maximum. Fraud detection is pattern-based and tolerates D1's eventual
  consistency for signal collection. The write-before-read pattern ensures the
  current request's data is always visible to its own signal queries.
</Aside>
...
The ML Worker uses two cron triggers to run three background tasks:

| Cron Expression  | Schedule           | Task                                                                                              |
| ---------------- | ------------------ | ------------------------------------------------------------------------------------------------- |
| `0 */6 * * *`    | Every 6 hours      | Refresh disposable domain list from external GitHub sources (71k+ domains stored in KV)           |
| `0 */6 * * *`    | Every 6 hours      | Prune validation records older than 30 days from D1 (GDPR compliance)                             |
| `0 3 * * SUN`    | Sundays 3AM UTC    | Automated model retraining via `TrainerContainer` Durable Object (see training pipeline below)    |
...
export default {
  async scheduled(
    ...
    // Domain refresh + data pruning run on every cron trigger
    ctx.waitUntil(updateDisposableDomains(env));
    ctx.waitUntil(pruneOldValidations(env.DB, 30));

    // Weekly retraining runs only on the Sunday schedule
    if (controller.cron === "0 3 * * SUN") {
      ctx.waitUntil(triggerContainerRetraining(env));
    }
  },
};
```

The weekly retraining task sends training parameters (`nTrees: 20, maxDepth: 6, minSamplesLeaf: 20, conflictWeight: 20`) to the `TrainerContainer` Durable Object, which runs a full pipeline: fetch live training data from D1, train a new Random Forest, run guardrails, and upload the validated model to KV. After success, training samples older than 14 days are pruned.

---

## Model training pipeline

The Random Forest is trained through two complementary pipelines: an **automated container-based pipeline** that retrains weekly on live production data, and an **offline CLI pipeline** for initial training and major model revisions.

### Container-based retraining (production)

A weekly cron (Sundays 3AM UTC) triggers the `TrainerContainer` Durable Object, which runs a Bun-based Docker container. The container:

1. Fetches the current training dataset from the Worker's D1 database (auto-collected feature vectors from every `/validate` call)
2. Trains a new Random Forest using a TypeScript CART implementation with Gini impurity, random feature subsampling, and conflict-zone weighting
3. Applies Platt calibration via Newton-Raphson optimization on OOB predictions
4. Runs guardrails (`recall >= 95%`, `FPR <= 5%`, `FNR <= 5%`)
5. Uploads the validated model to KV (propagates globally within 60 seconds)
6. Prunes training samples older than 14 days

If guardrails fail, the existing model remains active and a training failure event is logged.

### Offline CLI pipeline (development)

For initial training and major model changes, an offline Python/CLI toolchain produces the model artifact.
...
<Aside type="caution" title="No PII in training data">
  The canonical dataset (`main.csv`) contains only synthetic addresses. The
  Enron corpus provides email-address-level data only (no message content).
  Feature exports strip the email column by default.
</Aside>
...
digraph {
  ...
  Enron  [label="Enron 172K", shape=note];
  SynthL [label="Synth 327K", shape=note];
  SynthF [label="Synth 500K", shape=note];
  CSV    [label="main.csv 1M", shape=note];
  Export [label="features:export"];
  Feats  [label="features.csv", shape=note];
  Train  [label="RF 20 trees", shape=hexagon];
  Cal    [label="Platt Cal"];
  Guard  [label="Guardrails", shape=diamond];
  KV     [label="Upload KV", shape=cylinder];
  Worker [label="Worker ~60s", shape=ellipse];
  ...
  Cal    -> Guard;
  Guard  -> KV     [label="pass"];
  Guard  -> Train  [label="fail", style=dashed];
  KV     -> Worker;
}
```

**Step 1: Feature export** - Extract 48-feature vectors from the canonical dataset:
...
python cli/commands/model/train_forest.py \
  ...
  --n-trees 20 --max-depth 6 --min-samples-leaf 20 \
...
{
  "meta": {
    "version": "5.0.0-48feat",
    "features": ["avg_segment_length", "bigram_entropy", "digit_ratio", "...48 total..."],
    "tree_count": 20,
    "nTrees": 20,
    "maxDepth": 6,
    "feature_importance": {
      "provider_is_disposable": 0.160,
      "domain_reputation_score": 0.142,
      "digit_position_ratio": 0.119,
      "digit_ratio": 0.072,
      "name_token_overlap": 0.057,
      "max_digit_run": 0.056,
      "bigram_entropy": 0.051,
      "has_year_suffix": 0.047,
      "provider_is_free": 0.040,
      "entropy_score": 0.033
    },
    "calibration": {
      "method": "platt",
  ...
  "forest": ["...20 serialized decision trees..."]
}
```

Top 5 features by importance: disposable domain flag (0.160), domain reputation (0.142), digit position ratio (0.119), digit ratio (0.072), name-token overlap (0.057). The `digit_position_ratio` feature - measuring where digits appear relative to the local part length - was introduced in the 48-feature model and ranks 3rd by importance, demonstrating its value for distinguishing `john1990@` (birth year at end) from `2024signup@` (timestamp at start). Geo features (language/timezone mismatch) have near-zero importance in the current model - they contribute to heuristic overrides instead.
...
The ML Worker includes an Astro 5 + React 19 + Tailwind CSS 4 analytics dashboard (19 components) for monitoring fraud detection in production:

- **Metrics grid**: Key KPIs (total validations, block rate, warn rate, avg score)
- **Time-series charts**: Score distributions and decision trends over time (Recharts)
...
- **Review queue**: Manual label review for training sample correction
- **Training panel**: Training pipeline control and status monitoring
- **Threshold tuner**: Interactive threshold adjustment UI
...
Authentication uses HMAC-signed session cookies (SHA-256 derived signing key) with 24-hour TTL, `HttpOnly`/`Secure`/`SameSite=Strict` flags, and timing-safe comparison. Login is rate-limited to 5 attempts per minute per IP.
...
| **Add an ML feature**     | 1. Implement extractor in detector module 2. Add to `buildFeatureVector()` in `feature-vector.ts` 3. Run `features:export` to regenerate CSV 4. Retrain with `train_forest.py` (offline) or trigger container retraining 5. Run guardrails 6. Deploy model JSON to KV (no Worker redeploy) |
...
## From scoring to operations

A multi-layer scorer answers one question well: *how risky is this single
submission?* Turning that into an **abuse-operations** capability means answering a
different set of questions - about actors, campaigns, and what happens after a block.
The dimensions below mark where scoring stops and operations begins. Each is a generic
pattern; the table's middle column shows where a scoring-first system like this one
typically sits.

<Aside type="note" title="The pivot: per-event vs per-actor">
The deepest distinction is the **unit of analysis**. A scorer judges each submission in
isolation; its only cross-submission memory tends to be a blacklist. Abuse operations
treats the **campaign** - one actor or kit seen across many submissions - as the unit of
work. Most dimensions below follow from making that shift.
</Aside>

### Event-scoring vs actor-centric operations

| Dimension | What a scoring-only system misses | Pattern that closes it |
| --- | --- | --- |
| **Campaign / actor footprints** | It can't see that today's submission is yesterday's actor with one signal changed. | A *footprint* - a set of individually weak indicators identifying a campaign - matched **N-of-M**: a full match auto-actions, a partial match flags a likely **pivot** (same actor, changed tactic) for review. |
| **Intent classification** | A 0-100 score captures *how bad*, never *what kind* (monetisation, weaponisation, disruption, reputation-gaming, victimisation). | Tag each detection with an **intent aspect** so output is routable and explainable, not a single magnitude. |
| **A review tier** | One block/allow threshold forces every ambiguous case into either a false positive or a miss. | A third **review** band routes mid-confidence cases to a human queue - "is it good, or just not-bad-yet?" |
| **Return tracking** | If banned-actor records are deleted when a timeout lapses, a returning actor looks brand-new and any escalation resets. | Operations **reduce but never fully delete** return-detection data, so repeat offenders keep accumulating history across windows. |
| **Non-repetition** | Learning a new pattern only protects against *future* instances. | A **retroactive sweep** re-runs a newly learned footprint over recent history to catch stragglers - which means retaining enough raw signal to re-evaluate after the fact. |
| **Outbound = inbound** | Blocking protects the platform; it does nothing for the victim downstream. | **Devalue the loot**: report/reset harvested credentials, notify other providers in a distributed kit. A merely-blocked payload is still live elsewhere. |
| **External signal** | A self-generated blacklist only learns from abuse that already reached you. | Ingest **external feeds** (disposable-email lists, known-bad IPs/JA4) to act on abuse "seen elsewhere first" - legitimate pre-emption, not pre-crime. |

### What scoring-first already gets right

Two principles a well-built scorer satisfies almost by construction:

- **Reversibility** - a non-zero false-positive rate is unavoidable, so remediations should
  be undoable. Escalating timeouts and soft state beat permanent deletes.
- **Codifiability** - every decision carries an audit trail and rules live in
  configuration, never in manual database edits. This makes actions consistent,
  auditable, and safe to automate.

The system documented above does both: progressive (self-reversing) timeouts, a full
per-decision `ScoringDecision` audit trail, and hot-reloadable config and heuristics.

<Aside type="tip" title="Where to start">
The cheapest capabilities to add are the ones that only **log or label** - return
tracking, intent tags, a review tier. They change a stored record, not the request path.
The expensive part is always the **human workflow** around them: a queue an analyst works
through is far more effort than the detection that fills it. Build the headless signal
first; add the operator UI once the signal has proven itself.
</Aside>

[^cf-service-bindings]: Cloudflare, "Service bindings," Cloudflare Docs. https://developers.cloudflare.com/workers/runtime-apis/bindings/service-bindings/
[^cf-workers-rpc]: Cloudflare, "Workers RPC," Cloudflare Docs. https://developers.cloudflare.com/workers/runtime-apis/rpc/
