...
async function fetchWithKV(originUrl: string, env: Env): Promise<Response> {
  ...
  // Check KV first
  const cached = await env.CACHE_KV.get<CachedResponse>(cacheKey, 'json');
  if (cached) {
    return new Response(Uint8Array.from(atob(cached.body), c => c.charCodeAt(0)), {
  ...
  // Non-blocking write
  env.CACHE_KV.put(cacheKey, JSON.stringify(toCache), { expirationTtl: 3600 });
  
  return new Response(body, {
    status: response.status,
    headers: { 'Content-Type': toCache.contentType },
  });
}
```
...
export default {
  async fetch(request: Request): Promise<Response> {
    ...
    const upstream = await fetch('https://api.example.com/data', {
      cf: {
        cacheTtl: 3600,
        cacheEverything: true, // Required - JSON is not cached by default
      },
    });
