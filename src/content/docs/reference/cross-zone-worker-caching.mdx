...
When a Worker fetches without `cf` options, Cloudflare's [default caching](https://developers.cloudflare.com/cache/concepts/default-cache-behavior/) applies:
...
| Response has `Set-Cookie` header | No | Never cached |
...
export default {
  async fetch(request: Request): Promise<Response> {
    ...
    // Note: This alone doesn't enable caching for JSON without cacheEverything
    // or a Cache Rule - it only controls TTL if caching is enabled
...
**Important:** Just setting `Cache-Control` headers doesn't guarantee caching. For non-default content types (HTML, JSON, API responses), you also need:
- `cacheEverything: true` in `cf` options, OR
- A [Cache Rule](https://developers.cloudflare.com/cache/how-to/cache-rules/) with "Eligible for cache" or "Cache Everything"
