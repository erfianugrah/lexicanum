...
| `SECURITY DEFINER` helper | Join runs off the RLS path; pin `search_path` | Policy needs another table's data (see Supabase RLS guidance[^sb-rls]) |
...
policies filter on first, then wrap the helpers.
