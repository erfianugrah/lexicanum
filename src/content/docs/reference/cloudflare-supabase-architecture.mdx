...
- **A Durable Object runs wherever it was first created** unless you constrain it. `env.NS.jurisdiction("eu")` pins a DO's execution and storage to the EU (also `us`, `fedramp`); the Worker still reaches it from anywhere, only the DO's location is fixed. A **location hint** (`weur`, `apac`, ...) is best-effort *latency* placement and NOT a compliance guarantee - jurisdiction is. If a DO carries regulated data (the relay's broadcast payload is your row data), pin its jurisdiction to match the database. Pasteriser's relay carries only the public recent feed, so it is low-sensitivity - but it is exactly the DO you would pin if the payload were regulated. The `DurableObjectId` is still logged outside the jurisdiction for billing.
