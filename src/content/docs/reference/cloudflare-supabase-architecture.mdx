...
Channels are private by default and authorized by RLS policies on `realtime.messages`; the client sets `private: true` and re-sends its JWT on refresh, so keep JWT expiry short.
...
- **Durable Object relay** - a DO holds ONE server-side subscription and fans out to browsers over a **same-origin** socket. Keeps the browser -> Worker -> Supabase boundary intact (no key in the browser, CSP stays `connect-src 'self'`, one surface). The DO's hibernatable WebSocket API carries thousands of connections with no duration billing while idle.
...
- **Rejoin with Broadcast `replay`.** On reconnect, send `broadcast.replay { since, limit }` so anything broadcast during the gap is backfilled; dedup on the client by id (the browser socket stays up while only the DO's upstream reconnects, so a poll fallback would not cover it).
