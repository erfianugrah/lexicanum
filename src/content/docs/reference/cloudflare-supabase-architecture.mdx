...
- **The CF -> Supabase link is public-internet TLS**, whether PostgREST or Hyperdrive. Supabase **PrivateLink** (AWS PrivateLink, keeping connections off the public internet) applies to clients *inside* the same AWS network, not to a Cloudflare Worker - do not assume the Worker path is private.
...
- Do not treat the Worker -> Supabase hop as private - it is public-internet TLS, not PrivateLink.
