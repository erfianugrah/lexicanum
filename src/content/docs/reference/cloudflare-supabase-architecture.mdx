...
description: The full integration surface between Cloudflare Workers and Supabase — data access (REST vs Hyperdrive vs raw driver), writes, Auth (edge JWT verify vs getUser), Storage, Realtime, the security model, and per-PR preview branches wired to a Worker preview (including the MIGRATIONS_FAILED trap) — every path benchmarked from a Worker on CF's edge
author: Erfi Anugrah
---

import { Aside } from "@astrojs/starlight/components";

Every way a Cloudflare Worker talks to Supabase, which path to use for what, and what each one actually costs.
