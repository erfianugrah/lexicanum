...
| Real SQL (joins, transactions, `COPY`) or faster writes | **Hyperdrive** | ~10ms cached, ~172ms write | only working wire path from a Worker; you own authz |
...
- **Postgres Changes** streams row changes from the WAL to subscribers. It authorizes *per subscriber per event* and is single-threaded, so throughput scales with subscriber count, not compute - past ~3,000 concurrent subscribers, move off it.
...
- **The DO subscribes with the anon *JWT*, not the `sb_publishable_` key.** Realtime silently ignores `sb_*` tokens as a private-channel `access_token`, so the join is unauthenticated and receives nothing.
...
- A **Supabase Edge Function** (Deno) runs co-located with the Supabase services. Its DB, Auth, and Storage calls are effectively local (no ~210ms wire), but it is far from the user and has none of the CF primitives.

Put logic next to whatever it talks to most. A function that makes many DB calls and little user I/O is cheaper as an Edge Function (or a Worker with Smart Placement pointed at the DB region); a function that is user-facing, needs the Cache API / a Durable Object / R2, or is part of the CF stack is a Worker. They are not exclusive - a Worker at the edge can invoke an Edge Function for the DB-heavy step.

<Aside type="caution">
Edge Functions enforce a platform `verify_jwt` check *before* your handler.
When you rotate to asymmetric (ES256) signing keys, functions with `verify_jwt`
on break until you turn it off and verify with `getClaims()` - the same JWKS
rotation trap that hits any edge JWT verifier.
</Aside>
