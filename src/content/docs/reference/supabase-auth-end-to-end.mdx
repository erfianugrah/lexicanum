...
| Run the auth server yourself on a managed project | Self-hosted GoTrue as `postgres` with `search_path=auth`, legacy HS256 secret, platform ES256 public key as a verify-only JWK | this doc, [Taking Auth in-house](#taking-auth-in-house-a-self-hosted-gotrue-on-a-managed-project) |
...
**Not settled.** Registering the self-hosted GoTrue as a third-party issuer in its own right (so trust would rest on an ES256 key you own rather than the shared secret) needs its JWKS reachable from the platform; the container here was local. Pooler behaviour under load, an image ahead of the platform's migration set (which would try to write `auth.schema_migrations` as `postgres` and fail), SMTP, hooks and OAuth providers on the self-hosted side were not run.
